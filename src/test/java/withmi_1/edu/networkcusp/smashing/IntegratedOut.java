package withmi_1.edu.networkcusp.smashing;
 ...
 *  
 *  Modified by rborbely - package name changed, made methods non-static, and adapted for use 
 *  with any OutputStream
 */
public final class IntegratedOut {
   ...
   /**
    ...
    public void write(char x, int r) {
        ...
        for (int p = 0; p < r; ) {
            for (; (p < r) && (Math.random() < 0.5); ) {
                for (; (p < r) && (Math.random() < 0.4); ) {
                    for (; (p < r) && (Math.random() < 0.4); p++) {
