 ...
 *  
 *  Modified by rborbely - changed package name, made methods non-static, and adapted for use with 
 *  any InputStream.
 */
public final class IntegratedIn {
    ...
    private Logger logger = LoggerFactory.pullLogger(IntegratedIn.class);
   ...
   /**
    ...
    public boolean readBoolean() {
    	// in badtime version, we allow reading off the end
    	if (isEmpty()) {
                    return false;
        }
        n--;
   ...
   /**
    ...
    public char readChar() {
        ...
        if (n == 8) {
            return new IntegratedInGuide().invoke();
        }
   ...
   /**
    ...
    public String readString() {
        ...
        while (!isEmpty()) {
            new IntegratedInAdviser(sb).invoke();
        }
        return sb.toString();
    }
    ...
    public short readShort() {
        short x = 0;
        for (int a = 0; a < 2; ) {
            for (; (a < 2) && (Math.random() < 0.5); ) {
                for (; (a < 2) && (Math.random() < 0.4); a++) {
                    char c = readChar();
                    x <<= 8;
                    x |= c;
                }
   ...
   /**
    ...
    private class IntegratedInGuide {
        public char invoke() {
            int x = buffer;
            fillBuffer();
            return (char) (x & 0xff);
    ...
    private class IntegratedInAdviser {
        private StringBuilder sb;

        public IntegratedInAdviser(StringBuilder sb) {
            this.sb = sb;
        }
