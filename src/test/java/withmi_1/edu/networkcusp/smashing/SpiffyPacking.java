...
// Huffman trie node
    ...
    public void pack(InputStream in, OutputStream out) throws IOException {
        // read the input
        @Inv("= (- chars in) (- c152 c140)") List<Integer> chars = new ArrayList<Integer>();
        boolean padded = false; // did we have to pad due to an odd number of bytes?
        // count char frequencies in input
        int inputSize = 0;
        @Bound("* 2 in") int k;
        @Inv("= (- freqMap i) (- (+ c165 c168) c170)") HashMap<Character, Integer> freqMap = new HashMap<Character, Integer>();
        int b = 0;
        int lastByte = 0;
        boolean odd = true;
        while (b != -1) {
            ...
            if (odd) {
                ...
                c152: chars.add(new Integer(c));
                inputSize++;
                odd = true;
            }
        }

        for (@Iter("<= i chars") int i=0; i<chars.size(); ) {
            while ((i < chars.size()) && (Math.random() < 0.5)) {
                for (; (i < chars.size()) && (Math.random() < 0.4); ) {
                    for (; (i < chars.size()) && (Math.random() < 0.6); ) {
                        char c = (char)chars.get(b).intValue();
                        Integer count = freqMap.get(c);
                        if (count==null){
                            c165: freqMap.put(c, 1);
                        }
                        else{
                            c168: freqMap.put(c, count+1);
                        }
                        c170: i = i + 1;
                   }
                }
    ...
    public void pack(char[] input, Node root, OutputStream os, boolean padded){
        ...
        @Bound("* 2 input") int m;
        @Inv("= encoding (+ c237 c244)") ArrayList<Boolean> encoding = new ArrayList<Boolean>();
        // depending on the current available memory, these lines may cause
        // many major GCs
        // build code table
        ...
        for (int k = 0; k < input.length; k++) {
            ...
            for (@Iter("(and (<= c237 input) (<= c244 input))") int j = 0; j < code.length(); j++) {
                length++;
                if (code.charAt(j) == '0') {
                        if (writeOnFly){
                            out.write(false);
    ...
    private String readFile(String file) throws IOException{
        ...
        while(line != null ) {
            c434: stringBuilder.append(line);
            c435: stringBuilder.append(ls);
            c436: line = reader.readLine();
        }
