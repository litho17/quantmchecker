...
import withmi_1.edu.networkcusp.protocols.CommunicationsClient;
...
import withmi_1.edu.networkcusp.protocols.CommunicationsServer;
import withmi_1.edu.networkcusp.terminal.*;
import withmi_1.edu.networkcusp.chatbox.exception.WithMiFailure;
...
public class HangIn implements CommunicationsGuide {
    ...
    public void run() throws Throwable {
        ...
        try {
            ...
            @Bound("memberConductor.nameToMember") int j;
            @Inv("= (- members it) (- c116 c115)") List<WithMiUser> members = new ArrayList<>();
            @Iter("<= it memberConductor.nameToMember") Iterator<WithMiUser> it = memberConductor.nameToMember.values().iterator();
            while (it.hasNext()) {
                WithMiUser u;
                c115: u = it.next();
                c116: members.add(u);
            ...
            members.removeAll(memberConductor.pastMembers);
            // List<WithMiUser> currentMembers = grabCurrentMembers();
            for (int i = 0; i < members.size(); i++) {
                runUtility(members, i);
            }
        } finally {
    ...
    public void deliverMessage(Chat.WithMiMsg.Builder msgBuilder, WithMiChat discussion) throws CommunicationsFailure {
        ...
        @Bound("discussion.members") int i;
        @Inv("= (- members it) (- c186 c185)") List<WithMiUser> members = new ArrayList<>();
