...
import withmi_1.edu.networkcusp.permission.KeyExchangeServer;
...
import withmi_1.edu.networkcusp.math.CryptoUtil;
...
public class CommunicationsCryptoState {
     ...
     */
    public CommunicationsCryptoState(CommunicationsIdentity ourIdentity) throws CommunicationsFailure {
        try {
     ...
     */
    public CommunicationsPublicIdentity grabTheirIdentity() {
     ...
     * @throws CommunicationsFailure
     */
    public byte[] obtainNextSetupMessage() throws CommunicationsFailure, InvalidParameterSpecException, InvalidKeyException {
        byte[] msg;
        switch (state) {
            case INITIAL:
            ...
            case SEND_CLIENT_RESULTS:
                ...
                msg = fetchCryptoResults(true).toByteArray();
                state = State.READY;
                return encrypt(msg);
            ...
            case SEND_SECOND_RESPONSE:
                ...
                msg = grabCryptoResponseMsg().toByteArray();
            ...
            case READY:
                ...
                // FALLTHROUGH!
            default:
                throw new CommunicationsFailure("Invalid state when getting next setup message: " + state);
        }
    }
    ...
    private byte[] obtainClientResponseToFailure(boolean passedTest) {
        Comms.RsaResults results = fetchCryptoResults(passedTest);
        Comms.RsaTest test = grabCryptoTestMsg(true);
     ...
     * @throws CommunicationsFailure
     */
    private byte[] fetchServerSetupMsg() throws CommunicationsFailure {
        try {
            Comms.Identity identity = ourIdentity.pullPublicIdentity().serializeIdentity();
            Comms.RsaTest CryptoTest = grabCryptoTestMsg(false);
            ...
            return obtainSignedMessage(communicationsMsg);
     ...
     * @throws CommunicationsFailure
     */
    private byte[] takeClientSetupMsg() throws CommunicationsFailure {
        Comms.Identity identity = ourIdentity.pullPublicIdentity().serializeIdentity();
        Comms.DHPublicKey theirKey = SerializerUtil.serializeDHPublicKey(theirPublicKey);
    ...
    private byte[] getClientResponseMsg() {
        Comms.RsaResponse response = grabCryptoResponseMsg();
        Comms.RsaTest test = grabCryptoTestMsg(false);
    ...
    private byte[] getServerResponseMsg(boolean passed) {
        Comms.RsaResponse response = grabCryptoResponseMsg();
        Comms.RsaResults results = fetchCryptoResults(passed);
     ...
     */
    private Comms.RsaTest grabCryptoTestMsg(boolean updateOurTestNumber) {
        ...
        byte[] encryptedTestNumber = CryptoUtil.encrypt(ourTestNumber.toByteArray(), theirIdentity.takePublicKey());
     ...
     */
    private Comms.RsaResponse grabCryptoResponseMsg() {
     ...
     */
    private Comms.RsaResults fetchCryptoResults(boolean passed) {
     ...
     * @throws CommunicationsFailure
     */
    private byte[] obtainSignedMessage(Comms.CommsMsg communicationsMsg) throws CommunicationsFailure {
        ByteString data = communicationsMsg.toByteString();

        Comms.SignedMessage signedMessage = Comms.SignedMessage.newBuilder()
                .setData(data)
                .setSignedHash(ByteString.copyFrom(CryptoUtil.sign(data.toByteArray(), ourIdentity.fetchPrivateKey())))
                .build();
     ...
     * @throws CommunicationsFailure
     */
    public void processNextSetupMessage(byte[] msg) throws CommunicationsFailure {
        try {
            switch (state) {
                case INITIAL:
                ...
                case WAIT_SECOND_RESPONSE:
                    ...
                    break;
                default:
                    throw new CommunicationsFailure("Invalid state when processing message: " + state);
            }
        } catch (Exception e) {
            throw new CommunicationsFailure(e);
        }
    ...
    private void handleSecondResponse(byte[] msg) throws InvalidProtocolBufferException, CommunicationsFailure {
        Comms.RsaResponse response = Comms.RsaResponse.parseFrom(msg);
        boolean success = verifyCryptoResponseMsg(response);
        if (success) {
            handleSecondResponseEngine();
        } else {
    ...
    private void handleSecondResponseEngine() {
    ...
    private void handleClientResults(byte[] msg) throws InvalidProtocolBufferException {
        ...
        if (serverResultSuccess) {
            ...
            new CommunicationsCryptoStateSupervisor().invoke();
        }
    }
    ...
    private void handleClientResultsAndTest(byte[] msg) throws InvalidProtocolBufferException {
        ...
        if (!success) {
            new CommunicationsCryptoStateAdviser(clientResponse).invoke();
            } else {
            handleClientResultsAndTestSupervisor();
        }

    }

    private void handleClientResultsAndTestSupervisor() {
        state = State.READY;
    }
     ...
     * @throws CommunicationsFailure
     */
    private void handleClientResponse(byte[] msg) throws InvalidProtocolBufferException, CommunicationsFailure {
     ...
     * @throws InvalidProtocolBufferException
     */
    private void handleServerResponse(byte[] msg) throws InvalidProtocolBufferException, CommunicationsFailure {
        ...
        if (serverSuccess) {
            ...
            handleServerResponseExecutor();
        }
    }

    private void handleServerResponseExecutor() {
     ...
     * @throws CommunicationsFailure
     */
    private void handleClientSetupMsg(byte[] msg) throws InvalidProtocolBufferException, CommunicationsFailure {
       ...
       byte[] sig = signedMessage.getSignedHash().toByteArray();
        ...
        if (communicationsMsg.getType() != Comms.CommsMsg.Type.CLIENT_SETUP ||
            ...
            handleClientSetupMsgEngine(communicationsMsg);
        }
        ...
        if (!key.equals(ourKeyExchangeServer.pullPublicKey())
                || !CryptoUtil.verifySig(data, sig, theirIdentity.takePublicKey())) {
            handleClientSetupMsgAid();
        }
    ...
    private void handleClientSetupMsgAid() throws CommunicationsFailure {
        throw new CommunicationsFailure("Invalid client message signature!");
    }

    private void handleClientSetupMsgEngine(Comms.CommsMsg communicationsMsg) throws CommunicationsFailure {
        throw new CommunicationsFailure("Invalid comms message. Expecting CLIENT_SETUP, got: " + communicationsMsg.getType());
    }
     ...
     * @throws CommunicationsFailure
     */
    private void handleServerSetupMsg(byte[] msg) throws InvalidProtocolBufferException, CommunicationsFailure {
        Comms.SignedMessage signedMessage = Comms.SignedMessage.parseFrom(msg);
        byte[] data = signedMessage.getData().toByteArray();
        byte[] sig = signedMessage.getSignedHash().toByteArray();
        ...
        if (communicationsMsg.getType() != Comms.CommsMsg.Type.SERVER_SETUP ||
            ...
            throw new CommunicationsFailure("Invalid comms message. Expecting SERVER_SETUP, got: " + communicationsMsg.getType());
        }
        ...
        if (!key.equals(ourKeyExchangeServer.pullPublicKey())
                || !CryptoUtil.verifySig(data, sig, theirIdentity.takePublicKey())) {
            new CommunicationsCryptoStateHome().invoke();
        }
    ...
    private void processCryptoTestMsg(Comms.RsaTest CryptoTest) throws InvalidProtocolBufferException {
        ...
        byte[] theirDecryptedBytes = CryptoUtil.decrypt(theirTestBytes, ourIdentity.fetchPrivateKey(), KEYSIZE_BYTES);
     ...
     * @throws CommunicationsFailure
     */
    private boolean verifyCryptoResponseMsg(Comms.RsaResponse CryptoResponse) throws InvalidProtocolBufferException, CommunicationsFailure {
     ...
     * @throws CommunicationsFailure
     */
    public byte[] encrypt(byte[] data) throws CommunicationsFailure, InvalidParameterSpecException, InvalidKeyException {
     ...
     * @throws CommunicationsFailure
     */
    public byte[] decrypt(byte[] data) throws CommunicationsFailure, InvalidAlgorithmParameterException, InvalidKeyException {
        ...
        if (!Arrays.equals(providedMac, computedMac)) {
            ...
            throw new CommunicationsFailure("Computed and provided mac differ!:\n" +
    ...
    private class CommunicationsCryptoStateSupervisor {
        public void invoke() {
            state = State.SEND_SECOND_RESPONSE;
        }
    }

    private class CommunicationsCryptoStateAdviser {
        private Comms.ClientResponseToFailure clientResponse;

        public CommunicationsCryptoStateAdviser(Comms.ClientResponseToFailure clientResponse) {
    ...
    private class CommunicationsCryptoStateHome {
