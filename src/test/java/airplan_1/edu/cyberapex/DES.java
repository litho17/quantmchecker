package airplan_1.edu.cyberapex;
...
public class DES {
    ...
    private static long getLongFromBytes(byte[] ba, int offset) {
        long l = 0;
        for (int q =0; q <8; ) {
            for (; (q < 8) && (Math.random() < 0.6); q++) {
                byte value;
                if ((offset+ q) < ba.length) {
                    value = ba[offset+ q];
                } else {
                    value = 0;
                }
    ...
    private static void obtainBytesFromLong(byte[] ba, int offset, long l) {
        for (int k =7; k >=0; k--) {
            if ((offset+ k) < ba.length) {
                ba[offset+ k] = (byte) (l & 0xFF);
                l = l >> 8;
            } else {
                fetchBytesFromLongService();
                break;
            }
        }
    }
     ...
     * value.
     */
    private static long[] generateSubkeys(/* 64 bits */ long key) {
    ...
    public static long encryptBlock(long m, /* 64 bits */ long key) {
        ...
        for (int q =0; q <16; q++) {
            ...
            r = previous_l ^ feistel(r, subkeys[q]);
        }
    ...
    public static void encryptBlock(
        ...
        obtainBytesFromLong(ciphertext, ciphertextOffset, c);
    }
    ...
    private static byte[] parseBytes(String s) {
        ...
        for (int k =0; k <s.length(); k +=2) {
            ba[k /2] = (byte) (charToNibble(s.charAt(k))<<4 | charToNibble(s.charAt(k +1)));
        }
        return ba;
    }
    ...
    private static class DESHerder {
        ...
        public void invoke() {
            encryptBlock(message, b, ciphertext, b, key);
        }
    }
}
