...
public class MyPriorityQueue {
    ...
    public PriorityNode poll() {
        ...
        map.remove(first.fetchId()); // remove returned item from set
        return first;
    }
    ...
    public boolean addIfUseful(PriorityNode t) {
        int id = t.fetchId();
        if (!map.containsKey(id) || (t.grabRank() < map.get(id).grabRank())) { // if we already have a node with the same id
            return addIfUsefulAssist(t, id);
        }
        return false;
    }
    ...
    public void remove(PriorityNode t) {
        map.remove(t.fetchId()); // we can't remove it from the queue, but we'll know to ignore it if it's in there
    }

    public boolean contains(PriorityNode t) {
        return map.containsKey(t.fetchId());
    }
