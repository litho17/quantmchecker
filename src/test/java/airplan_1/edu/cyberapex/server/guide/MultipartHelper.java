...
public class MultipartHelper {
    ...
    public static Map<String, String> obtainMultipartFieldContent(HttpExchange httpExchange) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                obtainMultipartFieldContentTarget(postFields, iterator);
            }
        } catch (Exception e) {
            throw new IllegalArgumentException("Error parsing multipart message: " + e.getMessage(), e);
        }
    ...
    public static Map<String, List<String>> fetchMultipartValues(HttpExchange httpExchange,
        ...
        if (httpExchange == null) {
            return pullMultipartValuesTarget();
        }
            ...
            for (String fieldName : fieldNames) {
                ...
                if ((items != null) && !items.isEmpty()) {
                    ...
                    for (int p = 0; p < items.size(); p++) {
                        takeMultipartValuesHome(items, values, p);
                    }
                }
            }
        } catch (Exception e) {
        ...
        for (String fieldName : fields) {
            ...
            if ((items != null) && !items.isEmpty()) {
                ...
                for (int i = 0; i < items.size(); i++) {
                    extractFieldsFromParameterMapEngine(items, values, i);
                }
            }
        }
        return fieldNameValues;
    ...
    public static Map<String, String> grabMultipartFile(HttpExchange httpExchange,
        ...
        if (allFieldNames == null) {
            return takeMultipartFileGateKeeper();
        }
        ...
        try {
            ...
            Path path;
            if (fileName != null) {
                path = Paths.get(fileDestDir.toString(), fileName);
            } else {
                path = Paths.get(fileDestDir.toString(), fieldNameItems.get(fileFieldName));
            }
    ...
    public static List<String> fetchMultipartFieldItems(HttpExchange httpExchange, String fieldName) {
        ...
        if (StringUtils.isBlank(fieldName)) {
            return takeMultipartFieldItemsGuide();
        }
    ...
    private static class HttpExchangeRequestContext implements RequestContext {
        ...
        public HttpExchangeRequestContext(HttpExchange httpExchange) {
            if (!"POST".equals(httpExchange.getRequestMethod())) {
                HttpExchangeRequestContextGuide();
            }
            ...
            try {
                ...
                if (contentLength > MAX_POST_LENGTH) {
                    HttpExchangeRequestContextHelper();
                }
