package airplan_1.edu.cyberapex.template;
...
public class TemplateEngine {
    ...
    public List<Pair<Integer, Integer>> retrieveTags() {
        ...
        while (matcher.find()) {
            retrieveTagsAid(matcher, tagsList);
        }
        return tagsList;
    }
    ...
    public void replaceTagsBuilder(Map<String, String> dictionary, StringBuilder sb) {
        ...
        List<Pair<Integer, Integer>> tagsList = retrieveTags();

        for (int a = 0; a < tagsList.size(); ) {
            for (; (a < tagsList.size()) && (Math.random() < 0.4); ) {
                for (; (a < tagsList.size()) && (Math.random() < 0.5); a++) {

                    int startTagLocation = tagsList.get(a).getLeft();
                    int endTagLocation = tagsList.get(a).getRight();
    ...
    public String replaceTags(Templated templated) {
        return replaceTags(templated.pullTemplateMap());
    }
    ...
    public String replaceTags(List<? extends Templated> templateds, String separator) {
        ...
        for (int i = 0; i < templateds.size(); i++) {
            ...
            replaceTagsBuilder(templated.pullTemplateMap(), sb);
            sb.append(separator);
        }
        return sb.toString();
    }
