...
public class DES {
     ...
     * padding.
     */
    private static long fetchLongFromBytes(byte[] ba, int offset) {
        long l = 0;
        for (int p = 0; p <8; p++) {
            byte essence;
            if ((offset+ p) < ba.length) {
                essence = ba[offset+ p];
            } else {
                essence = 0;
            }
     ...
     */
    private static void pullBytesFromLong(byte[] ba, int offset, long l) {
        for (int i=7; i>=0; i--) {
...
//                break;
     ...
     * value.
     */
    private static long[] formSubkeys(/* 64 bits */ long key) {
    ...
    public static long encryptBlock(long m, /* 64 bits */ long key) {
        ...
        for (int c = 0; c <16; c++) {
            ...
            r = previous_l ^ feistel(r, subkeys[c]);
        }
    ...
    public static void encryptBlock(
        ...
        long m = fetchLongFromBytes(message, messageOffset);
        long k = fetchLongFromBytes(key, 0);
        long c = encryptBlock(m, k);
        pullBytesFromLong(ciphertext, ciphertextOffset, c);
    }
    ...
    public static byte[] encrypt(byte[] message, byte[] key) {
        ...
        for (int a = 0; a <message.length; a +=8) {
            encryptBlock(message, a, ciphertext, a, key);
        }
    ...
    private static byte[] parseBytes(String s) {
        ...
        for (int b = 0; b <s.length(); b +=2) {
            ba[b /2] = (byte) (charToNibble(s.charAt(b))<<4 | charToNibble(s.charAt(b +1)));
        }
        return ba;
    }
    ...
    public static boolean test(byte[] message, byte[] expected, byte[] key) {
        ...
        boolean outcome = Arrays.equals(expected, received);
        System.out.println("\tverdict: "+(outcome ?"PASS":"FAIL"));
        return outcome;
    }
