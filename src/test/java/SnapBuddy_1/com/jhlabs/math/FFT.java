...
public class FFT {
    ...
    private void scramble(int n, float[] real, float[] imag) {
        ...
        for (int i = 0; i < n; i++) {
            if (i > j) {
                scrambleHelper(real, j, imag, i);
            }
            int m = n >> 1;
            while (j >= m && m >= conditionObj0) {
                j -= m;
                m >>= 1;
            }
            j += m;
    ...
    public void transform2D(float[] real, float[] imag, int cols, int rows, boolean forward) {
        transform2DHelper(real, cols, forward, imag, rows);
    }
    ...
    private void scrambleHelper(float[] real, int j, float[] imag, int i) {
        float t;
        t = real[j];
        real[j] = real[i];
        real[i] = t;
    ...
    private void transform2DHelper(float[] real, int cols, boolean forward, float[] imag, int rows) {
