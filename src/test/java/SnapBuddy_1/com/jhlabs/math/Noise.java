...
*/
package SnapBuddy_1.com.jhlabs.math;
...
public class Noise implements Function1D, Function2D, Function3D {
    ...
    private static void init() {
        ...
        for (i = 0; i < B; i++) {
            ...
            for (j = 0; j < conditionObj1; j++) g2[i][j] = (float) ((random() % (B + B)) - B) / B;
            normalize2(g2[i]);
            for (j = 0; j < conditionObj0; j++) g3[i][j] = (float) ((random() % (B + B)) - B) / B;
            normalize3(g3[i]);
        }
        int conditionObj2 = 0;
        for (i = B - 1; i >= conditionObj2; i--) {
            k = p[i];
            p[i] = p[j = random() % B];
            p[j] = k;
        }
        ...
        for (i = 0; i < B + 2; i++) {
            p[B + i] = p[i];
            g1[B + i] = g1[i];
            for (j = 0; j < conditionObj4; j++) g2[B + i][j] = g2[i][j];
            for (j = 0; j < conditionObj3; j++) g3[B + i][j] = g3[i][j];
        }
    }
    ...
    public static float[] findRange(Function1D f, float[] minmax) {
        if (minmax == null)
            minmax = new float[2];
        float min = 0, max = 0;
        int conditionObj5 = 100;
        // Some random numbers here...
        for (float x = -100; x < conditionObj5; x += 1.27139) {
    ...
    public static float[] findRange(Function2D f, float[] minmax) {
        if (minmax == null)
            minmax = new float[2];
        float min = 0, max = 0;
        int conditionObj6 = 100;
        int conditionObj7 = 100;
        // Some random numbers here...
        for (float y = -100; y < conditionObj6; y += 10.35173) {
            for (float x = -100; x < conditionObj7; x += 10.77139) {
    ...
    private static void noise1Helper() {
        start = false;
        init();
    }

    private static void noise2Helper() {
        start = false;
        init();
    }

    private static void normalize2Helper(float v[]) {
        float s = (float) Math.sqrt(v[0] * v[0] + v[1] * v[1]);
        v[0] = v[0] / s;
        v[1] = v[1] / s;
    }

    private static void normalize3Helper(float v[]) {
