...
public class ShatterFilter extends AbstractBufferedImageOp {
    ...
    public void setTransition(float transition) {
        setTransitionHelper(transition);
    }
    ...
    public void setStartAlpha(float startAlpha) {
        setStartAlphaHelper(startAlpha);
    }
...
*/
        for (int y = 0; y < iterations; y++) {
            ...
            for (int x = 0; x < iterations; x++) {
                filterHelper(width, random, y1, cy, y2, cx, shapes, rz, height, y, rx, ry, x);
            }
        }
        for (int i = 0; i < numTiles; i++) {
            filterHelper1(g, width, numTiles, shapes, i, rz, height, src);
        }
        g.dispose();
        return dst;
    }
    ...
    private void setTransitionHelper(float transition) {
    ...
    private void setStartAlphaHelper(float startAlpha) {
        this.startAlpha = startAlpha;
    }

    private void setCentreYHelper(float centreY) {
        this.centreY = centreY;
    }

    private void filterHelper(float width, Random random, int y1, float cy, int y2, float cx, Tile[] shapes, float[] rz, float height, int y, float[] rx, float[] ry, int x) {
    ...
    private void filterHelper1(Graphics2D g, float width, int numTiles, Tile[] shapes, int i, float[] rz, float height, BufferedImage src) {
