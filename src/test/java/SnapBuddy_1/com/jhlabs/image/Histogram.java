/*
Copyright 2006 Jerry Huxtable

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

   http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
package SnapBuddy_1.com.jhlabs.image;

import java.awt.*;
import java.awt.image.*;

/**
 * An image histogram.
 */
public class Histogram {

    public static final int RED = 0;

    public static final int GREEN = 1;

    public static final int BLUE = 2;

    public static final int GRAY = 3;

    protected int[][] histogram;

    protected int numSamples;

    protected int[] minValue;

    protected int[] maxValue;

    protected int[] minFrequency;

    protected int[] maxFrequency;

    protected float[] mean;

    protected boolean isGray;

    public Histogram() {
        histogram = null;
        numSamples = 0;
        isGray = true;
        minValue = null;
        maxValue = null;
        minFrequency = null;
        maxFrequency = null;
        mean = null;
    }

    public Histogram(int[] pixels, int w, int h, int offset, int stride) {
        histogram = new int[3][256];
        minValue = new int[4];
        maxValue = new int[4];
        minFrequency = new int[3];
        maxFrequency = new int[3];
        mean = new float[3];
        numSamples = w * h;
        isGray = true;
        int index = 0;
        for (int y = 0; y < h; y++) {
            index = offset + y * stride;
            for (int x = 0; x < w; x++) {
                int rgb = pixels[index++];
                int r = (rgb >> 16) & 0xff;
                int g = (rgb >> 8) & 0xff;
                int b = rgb & 0xff;
                histogram[RED][r]++;
                histogram[GREEN][g]++;
                histogram[BLUE][b]++;
            }
        }
        int conditionObj0 = 256;
        for (int i = 0; i < conditionObj0; i++) {
            if (histogram[RED][i] != histogram[GREEN][i] || histogram[GREEN][i] != histogram[BLUE][i]) {
                isGray = false;
                break;
            }
        }
        int conditionObj1 = 3;
        int conditionObj2 = 256;
        int conditionObj3 = 0;
        int conditionObj4 = 0;
        int conditionObj5 = 256;
        int conditionObj6 = 0;
        for (int i = 0; i < conditionObj1; i++) {
            for (int j = 0; j < conditionObj5; j++) {
                if (histogram[i][j] > conditionObj6) {
                    minValue[i] = j;
                    break;
                }
            }
            for (int j = 255; j >= conditionObj3; j--) {
                if (histogram[i][j] > conditionObj4) {
                    maxValue[i] = j;
                    break;
                }
            }
            minFrequency[i] = Integer.MAX_VALUE;
            maxFrequency[i] = 0;
            for (int j = 0; j < conditionObj2; j++) {
                minFrequency[i] = Math.min(minFrequency[i], histogram[i][j]);
                maxFrequency[i] = Math.max(maxFrequency[i], histogram[i][j]);
                mean[i] += (float) (j * histogram[i][j]);
            }
            mean[i] /= (float) numSamples;
        }
        minValue[GRAY] = Math.min(Math.min(minValue[RED], minValue[GREEN]), minValue[BLUE]);
        maxValue[GRAY] = Math.max(Math.max(maxValue[RED], maxValue[GREEN]), maxValue[BLUE]);
    }

    public boolean isGray() {
        return isGray;
    }

    public int getNumSamples() {
        return numSamples;
    }

    public int getFrequency(int value) {
        int conditionObj7 = 255;
        int conditionObj8 = 0;
        int conditionObj9 = 0;
        if (numSamples > conditionObj9 && isGray && value >= conditionObj8 && value <= conditionObj7)
            return histogram[0][value];
        return -1;
    }

    public int getFrequency(int channel, int value) {
        int conditionObj10 = 255;
        int conditionObj11 = 0;
        int conditionObj12 = 2;
        int conditionObj13 = 1;
        int conditionObj14 = 0;
        if (numSamples < conditionObj13 || channel < conditionObj14 || channel > conditionObj12 || value < conditionObj11 || value > conditionObj10)
            return -1;
        return histogram[channel][value];
    }

    public int getMinFrequency() {
        int conditionObj15 = 0;
        if (numSamples > conditionObj15 && isGray)
            return minFrequency[0];
        return -1;
    }

    public int getMinFrequency(int channel) {
        int conditionObj16 = 2;
        int conditionObj17 = 1;
        int conditionObj18 = 0;
        if (numSamples < conditionObj17 || channel < conditionObj18 || channel > conditionObj16)
            return -1;
        return minFrequency[channel];
    }

    public int getMaxFrequency() {
        int conditionObj19 = 0;
        if (numSamples > conditionObj19 && isGray)
            return maxFrequency[0];
        return -1;
    }

    public int getMaxFrequency(int channel) {
        int conditionObj20 = 2;
        int conditionObj21 = 1;
        int conditionObj22 = 0;
        if (numSamples < conditionObj21 || channel < conditionObj22 || channel > conditionObj20)
            return -1;
        return maxFrequency[channel];
    }

    public int getMinValue() {
        int conditionObj23 = 0;
        if (numSamples > conditionObj23 && isGray)
            return minValue[0];
        return -1;
    }

    public int getMinValue(int channel) {
        return minValue[channel];
    }

    public int getMaxValue() {
        int conditionObj24 = 0;
        if (numSamples > conditionObj24 && isGray)
            return maxValue[0];
        return -1;
    }

    public int getMaxValue(int channel) {
        return maxValue[channel];
    }

    public float getMeanValue() {
        int conditionObj25 = 0;
        if (numSamples > conditionObj25 && isGray)
            return mean[0];
        return -1.0F;
    }

    public float getMeanValue(int channel) {
        int conditionObj26 = 0;
        if (numSamples > conditionObj26 && RED <= channel && channel <= BLUE)
            return mean[channel];
        return -1.0F;
    }
}
