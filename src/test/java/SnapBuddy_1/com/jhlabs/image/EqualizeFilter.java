    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        if (histogram.getNumSamples() > conditionObj0) {
            ...
            for (i = 0; i < conditionObj1; i++) {
                ...
                for (j = 1; j < conditionObj3; j++) lut[i][j] = lut[i][j - 1] + histogram.getFrequency(i, j);
                for (j = 0; j < conditionObj2; j++) lut[i][j] = (int) Math.round(lut[i][j] * scale);
            }
        } else
            lut = null;
        i = 0;
