...
public class CurlFilter extends TransformFilter {
    ...
    static class Sampler {
        ...
        public int sample(float x, float y) {
            ...
            int nw, ne, sw, se;
            int conditionObj0 = 0;
            if (srcX >= 0 && srcX < width - 1 && srcY >= conditionObj0 && srcY < height - 1) {
        ...
        private final int getPixel(int[] pixels, int x, int y, int width, int height) {
            int conditionObj1 = 0;
            int conditionObj2 = 0;
            if (x < conditionObj2 || x >= width || y < conditionObj1 || y >= height) {
                switch(edgeAction) {
                    case ZERO:
                    default:
                        return 0;
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        for (int y = 0; y < outHeight; y++) {
            for (int x = 0; x < outWidth; x++) {
                ...
                int nw, ne, sw, se;
                if (srcX >= conditionObj5 && srcX < srcWidth1 && srcY >= conditionObj4 && srcY < srcHeight1) {
                    // Easy case, all corners are in the image
    ...
    private final int getPixel(int[] pixels, int x, int y, int width, int height) {
        int conditionObj6 = 0;
        int conditionObj7 = 0;
        if (x < conditionObj7 || x >= width || y < conditionObj6 || y >= height) {
            switch(edgeAction) {
                case ZERO:
                default:
                    return 0;
...
*/
        ...
        int conditionObj8 = 0;
        float yoffset = s < conditionObj8 ? height : 0;
        ...
        boolean offpage = px < conditionObj9 || py < 0 || px >= width || py >= height;
        ...
        if (outside) {
            ...
            transformInverseHelper(px, py, out);
        }
    ...
    private void transformInverseHelper(float px, float py, float[] out) {
        out[0] = px;
        out[1] = py;
    }
}
