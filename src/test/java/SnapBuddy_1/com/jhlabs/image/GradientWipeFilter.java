...
public class GradientWipeFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                filterHelper(lower, inPixels, maskWidth, upper, maskPixels, x);
            }
    ...
    private void setMaskHelper(BufferedImage mask) {
        this.mask = mask;
    }

    private void filterHelper(float lower, int[] inPixels, int maskWidth, float upper, int[] maskPixels, int x) {
