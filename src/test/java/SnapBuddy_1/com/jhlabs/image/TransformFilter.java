...
public abstract class TransformFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        for (int y = 0; y < outHeight; y++) {
            for (int x = 0; x < outWidth; x++) {
                filterHelper(outY, outX, inPixels, srcWidth, srcWidth1, conditionObj0, conditionObj1, outPixels, srcHeight, srcHeight1, y, out, x);
            }
    ...
    private final int getPixel(int[] pixels, int x, int y, int width, int height) {
        int conditionObj2 = 0;
        int conditionObj3 = 0;
        if (x < conditionObj3 || x >= width || y < conditionObj2 || y >= height) {
            switch(edgeAction) {
                case ZERO:
                default:
                    return 0;
    ...
    protected BufferedImage filterPixelsNN(BufferedImage dst, int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        for (int y = 0; y < outHeight; y++) {
            for (int x = 0; x < outWidth; x++) {
                ...
                if (out[0] < conditionObj5 || srcX >= srcWidth || out[1] < conditionObj4 || srcY >= srcHeight) {
                    int p;
                    switch(edgeAction) {
                        case ZERO:
                        default:
    ...
    private void filterHelper(int outY, int outX, int[] inPixels, int srcWidth, int srcWidth1, int conditionObj0, int conditionObj1, int[] outPixels, int srcHeight, int srcHeight1, int y, float[] out, int x) {
        transformInverse(outX + x, outY + y, out);
        ...
        int nw, ne, sw, se;
        if (srcX >= conditionObj1 && srcX < srcWidth1 && srcY >= conditionObj0 && srcY < srcHeight1) {
