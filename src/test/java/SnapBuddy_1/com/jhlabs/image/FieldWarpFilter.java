...
public class FieldWarpFilter extends TransformFilter {
    ...
    protected void transformInverse(int x, int y, float[] out) {
        ...
        for (int line = 0; line < inLines.length; line++) {
            ...
            if (fraction <= conditionObj0)
                distance = (float) Math.sqrt(dx * dx + dy * dy);
            else if (fraction >= conditionObj1) {
                dx = x - l.x2;
                dy = y - l.y2;
                distance = (float) Math.sqrt(dx * dx + dy * dy);
            } else if (fdist >= conditionObj2)
                distance = fdist;
            else
                distance = -fdist;
    ...
    private void setInLinesHelper(Line[] inLines) {
        this.inLines = inLines;
    }

    private void filterHelper(int line) {
