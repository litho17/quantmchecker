...
*/
    ...
    public static int colorSpline(int x, int numKnots, int[] xknots, int[] yknots) {
        ...
        float k0, k1, k2, k3;
        float c0, c1, c2, c3;
        int conditionObj10 = 1;
        if (numSpans < conditionObj10)
            throw new  IllegalArgumentException("Too few knots in spline");
        for (span = 0; span < numSpans; span++) if (xknots[span + 1] > x)
            break;
        if (span > numKnots - 3)
        ...
        if (span < conditionObj11) {
            span = 0;
            t = 0;
        }
        int v = 0;
        int conditionObj12 = 4;
        int conditionObj13 = 0;
        int conditionObj14 = 255;
    ...
    public static void unpremultiply(int[] p, int offset, int length) {
        ...
        int conditionObj19 = 255;
        for (int i = offset; i < length; i++) {
            unpremultiplyHelper(conditionObj19, conditionObj17, conditionObj18, conditionObj15, p, conditionObj16, i);
        }
    }

    private static void unpremultiplyHelper(int conditionObj19, int conditionObj17, int conditionObj18, int conditionObj15, int[] p, int conditionObj16, int i) {
        int rgb = p[i];
        int a = (rgb >> 24) & 0xff;
        int r = (rgb >> 16) & 0xff;
        int g = (rgb >> 8) & 0xff;
        int b = rgb & 0xff;
        if (a != conditionObj18 && a != conditionObj19) {
            float f = 255.0f / a;
            r *= f;
            g *= f;
            b *= f;
