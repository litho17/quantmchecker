...
public class Gradient extends ArrayColormap implements Cloneable {
    ...
    public void setColor(int n, int color) {
        ...
        int conditionObj1 = 256;
        if (n < 256 - 1)
            for (int i = n; i < conditionObj1; i++) map[i] = ImageMath.mixColors((float) (i - n) / (256 - n), color, lastColor);
    }
    ...
    public void setKnots(int[] x, int[] y, byte[] types, int offset, int count) {
        setKnotsHelper(count, types, offset, y, x);
    }
    ...
    private void rebuildGradient() {
        ...
        for (int i = 1; i < numKnots - 1; i++) {
            ...
            for (int j = xKnots[i]; j < end; j++) {
                ...
                if (j >= conditionObj4 && j <= conditionObj5) {
                    switch(blend) {
                        case CONSTANT:
                            t = 0;
                            break;
    ...
    private void copyToHelper(Gradient g) {
        g.numKnots = numKnots;
        g.map = (int[]) map.clone();
        g.xKnots = (int[]) xKnots.clone();
        g.yKnots = (int[]) yKnots.clone();
    ...
    private void setKnotTypeHelper(int n, int type) {
        knotTypes[n] = (byte) ((knotTypes[n] & ~COLOR_MASK) | type);
        rebuildGradient();
    }

    private void addKnotHelper(int color, int type, int x) {
    ...
    private void setKnotsHelper(int count, byte[] types, int offset, int[] y, int[] x) {
        numKnots = count;
        xKnots = new int[numKnots];
        yKnots = new int[numKnots];
        knotTypes = new byte[numKnots];
    ...
    private void randomizeHelper(int i) {
    ...
    private void mutateHelper(float amount) {
        for (int i = 0; i < numKnots; i++) {
            int rgb = yKnots[i];
            int r = ((rgb >> 16) & 0xff);
            int g = ((rgb >> 8) & 0xff);
