...
public class BorderFilter extends AbstractBufferedImageOp {
    ...
    public void setBorderPaint(Paint borderPaint) {
        setBorderPaintHelper(borderPaint);
    }
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        int conditionObj3 = 0;
        if (borderPaint != null) {
            filterHelper(g, height, width, conditionObj0, conditionObj1, conditionObj2, conditionObj3);
        }
    ...
    private void setTopBorderHelper(int topBorder) {
        this.topBorder = topBorder;
    }

    private void setBorderPaintHelper(Paint borderPaint) {
        this.borderPaint = borderPaint;
    }

    private void filterHelper(Graphics2D g, int height, int width, int conditionObj0, int conditionObj1, int conditionObj2, int conditionObj3) {
        g.setPaint(borderPaint);
        if (leftBorder > conditionObj3)
            g.fillRect(0, 0, leftBorder, height);
        if (rightBorder > conditionObj2)
            g.fillRect(width - rightBorder, 0, rightBorder, height);
        if (topBorder > conditionObj1)
            g.fillRect(leftBorder, 0, width - leftBorder - rightBorder, topBorder);
        if (bottomBorder > conditionObj0)
