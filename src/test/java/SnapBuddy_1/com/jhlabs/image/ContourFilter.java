...
public class ContourFilter extends WholeImageFilter {
    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        for (int i = 0; i < 256; i++) table[i] = (short) PixelUtils.clamp((int) (255 * Math.floor(levels * (i + offsetl) / 256) / (levels - 1) - offsetl));
        for (int x = 0; x < width; x++) {
            filterPixelsHelper(inPixels, r, x);
        }
        int conditionObj0 = 0;
        ...
        for (int y = 0; y < height; y++) {
            boolean yIn = y > conditionObj3 && y < height - 1;
            ...
            for (int x = 0; x < width; x++) {
                boolean xIn = x > conditionObj2 && x < width - 1;
                int w = x - 1;
                int e = x + 1;
                int v = 0;
                if (yIn && xIn) {
    ...
    private void setOffsetHelper(float offset) {
        this.offset = offset;
    }

    private void filterPixelsHelper(int[] inPixels, short[][] r, int x) {
        int rgb = inPixels[x];
