...
public class WarpGrid {
    ...
    public void warp(int[] inPixels, int cols, int rows, WarpGrid sourceGrid, WarpGrid destGrid, int[] outPixels) {
        warpHelper(destGrid, outPixels, cols, inPixels, sourceGrid, rows);
    }
    ...
    private void addRowHelper(int l, int k, float[] y, float[] x) {
        x[k] = xGrid[l];
        y[k] = yGrid[l];
    }

    private void addColHelper(int before) {
        int size = rows * (cols + 1);
        float[] x = new float[size];
        float[] y = new float[size];
        cols++;
    ...
    private void removeColHelper(int r, float[] y, int row, float[] x) {
        int i = row * (cols + 1);
        int j = row * cols;
        for (int col = 0; col < cols; col++) {
            x[j] = xGrid[i];
    ...
    private void warpHelper(WarpGrid destGrid, int[] outPixels, int cols, int[] inPixels, WarpGrid sourceGrid, int rows) {
        try {
            int x, y;
            int u, v;
            int[] intermediate;
