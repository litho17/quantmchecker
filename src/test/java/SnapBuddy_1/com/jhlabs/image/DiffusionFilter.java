...
public class DiffusionFilter extends WholeImageFilter {
    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        for (int i = 0; i < conditionObj0; i++) div[i] = levels * i / 256;
        int conditionObj1 = 1;
        int conditionObj2 = 1;
        int conditionObj3 = 0;
        int conditionObj4 = 1;
        for (int y = 0; y < height; y++) {
            boolean reverse = serpentine && (y & 1) == conditionObj4;
            int direction;
            if (reverse) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                int eg = g1 - g2;
                int eb = b1 - b2;
                for (int i = -1; i <= conditionObj1; i++) {
                    int iy = i + y;
                    if (0 <= iy && iy < height) {
                        for (int j = -1; j <= conditionObj2; j++) {
                            int jx = j + x;
                            if (0 <= jx && jx < width) {
                                int w;
                                if (reverse)
    ...
    private void setLevelsHelper(int levels) {
        this.levels = levels;
    }
}
