...
public class VariableBlurFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        for (int i = 0; i < iterations; i++) {
            filterHelper(outPixels, height, inPixels, width);
        }
        if (premultiplyAlpha)
    ...
    public void blur(int[] in, int[] out, int width, int height, int radius, int pass) {
        ...
        for (int y = 0; y < height; y++) {
            int outIndex = y;
            if (blurMask != null) {
                blurHelper(conditionObj4, width, mask, pass, y);
            }
    ...
    private void filterHelper(int[] outPixels, int height, int[] inPixels, int width) {
        blur(inPixels, outPixels, width, height, hRadius, 1);
        blur(outPixels, inPixels, height, width, vRadius, 2);
    }

    private void blurHelper(int conditionObj4, int width, int[] mask, int pass, int y) {
        if (pass == conditionObj4)
            getRGB(blurMask, 0, y, width, 1, mask);
        else
            getRGB(blurMask, y, 0, 1, width, mask);
    }

    private void blurHelper1(int[] g, int[] b, int[] a, int[] r, int x) {
        a[x] += a[x - 1];
        r[x] += r[x - 1];
        g[x] += g[x - 1];
        b[x] += b[x - 1];
    ...
    private void setHRadiusHelper(int hRadius) {
        this.hRadius = hRadius;
    }

    private void setVRadiusHelper(int vRadius) {
