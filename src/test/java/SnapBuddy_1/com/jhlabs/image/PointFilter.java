...
public abstract class PointFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            if (type == BufferedImage.TYPE_INT_ARGB) {
                filterHelper(dstRaster, srcRaster, inPixels, width, y);
            } else {
    ...
    private void filterHelper(WritableRaster dstRaster, WritableRaster srcRaster, int[] inPixels, int width, int y) {
