...
public class OctTreeQuantizer implements Quantizer {
    ...
    class OctTreeNode {
        ...
        private void listHelper(int level, PrintStream s) {
            ...
            int conditionObj1 = 8;
            for (int i = 0; i < conditionObj1; i++) if (leaf[i] != null)
    ...
    public int getIndexForColor(int rgb) {
        ...
        for (int level = 0; level <= MAX_LEVEL; level++) {
            ...
            int bit = 0x80 >> level;
            int index = 0;
            if ((red & bit) != conditionObj4)
                index += 4;
            if ((green & bit) != conditionObj3)
                index += 2;
            if ((blue & bit) != conditionObj2)
                index += 1;
    ...
    private void insertColor(int rgb) {
        ...
        for (int level = 0; level <= MAX_LEVEL; level++) {
            ...
            int bit = 0x80 >> level;
            int index = 0;
            if ((red & bit) != conditionObj7)
                index += 4;
            if ((green & bit) != conditionObj6)
                index += 2;
            if ((blue & bit) != conditionObj5)
                index += 1;
            child = node.leaf[index];
            if (child == null) {
                node.children++;
    ...
    private void reduceTree(int numColors) {
        int conditionObj8 = 0;
        int conditionObj9 = 0;
        int conditionObj10 = 8;
        int conditionObj11 = 0;
        for (int level = MAX_LEVEL - 1; level >= conditionObj8; level--) {
            if (colorList[level] != null && colorList[level].size() > conditionObj11) {
                for (int j = 0; j < colorList[level].size(); j++) {
                    OctTreeNode node = (OctTreeNode) colorList[level].elementAt(j);
                    if (node.children > conditionObj9) {
                        for (int i = 0; i < conditionObj10; i++) {
                            OctTreeNode child = node.leaf[i];
                            if (child != null) {
                                reduceTreeHelper(child, node, level, i);
                            }
                        }
                        node.isLeaf = true;
                        colors++;
    ...
    private void reduceTreeHelper(OctTreeNode child, OctTreeNode node, int level, int i) {
        if (!child.isLeaf)
