...
import SnapBuddy_1.com.jhlabs.composite.*;
...
public class GlintFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        if (colormap != null) {
            ...
            for (int i = 0; i <= length2; i++) {
                filterHelper(length2, colors2, i);
            }
        }
        BufferedImage mask = new  BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        int threshold3 = (int) (threshold * 3 * 255);
        for (int y = 0; y < height; y++) {
            filterHelper1(threshold3, width, mask, src, pixels, y);
        }
        int conditionObj0 = 0;
        if (blur != conditionObj0)
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                if ((pixels[x] & 0xff) > threshold * 255) {
                    ...
                    // Horizontal
                    for (int i = 0, k = 0; i <= xmax; i++, k++) dstPixels[index + i] = PixelUtils.combinePixels(dstPixels[index + i], colors[k], PixelUtils.ADD);
                    for (int i = -1, k = 1; i >= xmin; i--, k++) dstPixels[index + i] = PixelUtils.combinePixels(dstPixels[index + i], colors[k], PixelUtils.ADD);
                    // Vertical
                    for (int i = 1, j = index + width, k = 0; i <= ymax; i++, j += width, k++) dstPixels[j] = PixelUtils.combinePixels(dstPixels[j], colors[k], PixelUtils.ADD);
                    for (int i = -1, j = index - width, k = 0; i >= ymin; i--, j -= width, k++) dstPixels[j] = PixelUtils.combinePixels(dstPixels[j], colors[k], PixelUtils.ADD);
                    // Diagonals
                    int xymin = Math.max(xmin2, ymin2);
                    int xymax = Math.min(xmax2, ymax2);
                    // SE
                    int count = Math.min(xmax2, ymax2);
                    for (int i = 1, j = index + width + 1, k = 0; i <= count; i++, j += width + 1, k++) dstPixels[j] = PixelUtils.combinePixels(dstPixels[j], colors2[k], PixelUtils.ADD);
                    // NW
    ...
    private void setThresholdHelper(float threshold) {
        this.threshold = threshold;
    }

    private void setColormapHelper(Colormap colormap) {
        this.colormap = colormap;
    }

    private void filterHelper(int length2, int[] colors2, int i) {
    ...
    private void filterHelper1(int threshold3, int width, BufferedImage mask, BufferedImage src, int[] pixels, int y) {
