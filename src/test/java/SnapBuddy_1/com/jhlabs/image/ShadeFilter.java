...
public class ShadeFilter extends WholeImageFilter {
    ...
    public void setBumpHeight(float bumpHeight) {
        setBumpHeightHelper(bumpHeight);
    }
    ...
    public void setBumpSource(int bumpSource) {
        setBumpSourceHelper(bumpSource);
    }
    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        int conditionObj0 = 0;
        boolean invertBumps = bumpHeight < conditionObj0;
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (bumpSource != BUMPS_FROM_BEVEL) {
                    ...
                    float m1 = x > conditionObj4 ? width45 * bump.evaluate(nx - 1.0f, ny) - m0 : -2;
                    float m2 = y > conditionObj3 ? width45 * bump.evaluate(nx, ny - 1.0f) - m0 : -2;
    ...
    private void setBumpHeightHelper(float bumpHeight) {
        this.bumpHeight = bumpHeight;
    }

    private void setEnvironmentMapHelper() {
        envWidth = envHeight = 1;
        envPixels = null;
    }

    private void setBumpSourceHelper(int bumpSource) {
