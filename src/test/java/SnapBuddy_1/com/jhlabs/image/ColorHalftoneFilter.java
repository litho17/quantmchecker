...
public class ColorHalftoneFilter extends AbstractBufferedImageOp {
    ...
    public void setMagentaScreenAngle(float magentaScreenAngle) {
        setMagentaScreenAngleHelper(magentaScreenAngle);
    }
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        for (int y = 0; y < height; y++) {
            filterHelper(gridSize, halfGridSize, inPixels, width, mx, dst, conditionObj0, my, conditionObj1, outPixels, angles, height, y);
        }
        return dst;
    }
    ...
    private void setdotRadiusHelper(float dotRadius) {
        this.dotRadius = dotRadius;
    }

    private void setMagentaScreenAngleHelper(float magentaScreenAngle) {
        this.magentaScreenAngle = magentaScreenAngle;
    }

    private void filterHelper(float gridSize, float halfGridSize, int[] inPixels, int width, float[] mx, BufferedImage dst, int conditionObj0, float[] my, int conditionObj1, int[] outPixels, float[] angles, int height, int y) {
        for (int x = 0, ix = y * width; x < width; x++, ix++) outPixels[x] = (inPixels[ix] & 0xff000000) | 0xffffff;
        for (int channel = 0; channel < conditionObj0; channel++) {
