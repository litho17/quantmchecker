...
public class ShapeFilter extends WholeImageFilter {
    ...
    public int distanceMap(int[] map, int width, int height) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (map[offset] > conditionObj0) {
                    if (x < conditionObj1 || x > xmax || y < 2 || y > ymax)
        ...
        int conditionObj5 = 2;
        int conditionObj6 = 2;
        for (int y = height - 1; y >= conditionObj2; y--) {
            for (int x = width - 1; x >= conditionObj3; x--) {
                int offset = x + y * width;
                if (map[offset] > conditionObj4) {
                    if (x < conditionObj6 || x > xmax || y < conditionObj5 || y > ymax)
                        v = setEdgeValue(x, y, map, width, offset, xmax, ymax);
                    else
                        v = setValue(map, width, offset);
                    if (v > max)
    ...
    private void makeMap(int[] pixels, int[] map, int width, int height) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                makeMapHelper(width, map, pixels, y, x);
            }
        }
    }
    ...
    private void applyMap(int[] map, int[] pixels, int width, int height, int max) {
        int conditionObj7 = 0;
        if (max == conditionObj7)
            max = 1;
        int conditionObj8 = 0;
        int conditionObj9 = 255;
        int conditionObj10 = 0;
        int conditionObj11 = 255;
    ...
    private int setEdgeValue(int x, int y, int[] map, int width, int offset, int xmax, int ymax) {
        ...
        if (y == conditionObj14 || x == conditionObj15 || y == ymax + 2 || x == xmax + 2)
        ...
        if (v < min)
            min = v;
        int conditionObj16 = 1;
        if (y == 1 || x == conditionObj16 || y == ymax + 1 || x == xmax + 1)
    ...
    private void makeMapHelper(int width, int[] map, int[] pixels, int y, int x) {
        int offset = x + y * width;
