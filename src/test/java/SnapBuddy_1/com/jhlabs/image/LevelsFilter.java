...
public class LevelsFilter extends WholeImageFilter {
    ...
    public void setLowOutputLevel(float lowOutputLevel) {
        setLowOutputLevelHelper(lowOutputLevel);
    }
    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        if (histogram.getNumSamples() > conditionObj0) {
            ...
            if (low == high)
                high++;
            for (i = 0; i < conditionObj1; i++) {
                for (j = 0; j < conditionObj2; j++) lut[i][j] = PixelUtils.clamp((int) (255 * (lowOutputLevel + (highOutputLevel - lowOutputLevel) * (j - low) / (high - low))));
            }
        } else
            lut = null;
        i = 0;
    ...
    private void setLowOutputLevelHelper(float lowOutputLevel) {
