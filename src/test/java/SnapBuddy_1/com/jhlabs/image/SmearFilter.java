...
public class SmearFilter extends WholeImageFilter {
    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        switch(shape) {
            ...
            case LINES:
                ...
                for (i = 0; i < numShapes; i++) {
                    filterPixelsHelper(conditionObj4, conditionObj5, conditionObj6, conditionObj7, inPixels, conditionObj8, width, cosAngle, conditionObj2, conditionObj3, outPixels, height, sinAngle);
                }
                break;
            case SQUARES:
            case CIRCLES:
                    ...
                    for (int x = sx - radius; x < sx + radius + 1; x++) {
                        for (int y = sy - radius; y < sy + radius + 1; y++) {
                            ...
                            if (shape == CIRCLES)
                                ...
                                f = 0;
                            if (x >= conditionObj1 && x < width && y >= conditionObj0 && y < height && f <= radius2) {
    ...
    private void setShapeHelper(int shape) {
        this.shape = shape;
    }

    private void setScatterHelper(float scatter) {
        this.scatter = scatter;
    }

    private void randomizeHelper() {
        seed = new  Date().getTime();
    }

    private void filterPixelsHelper(int conditionObj4, int conditionObj5, int conditionObj6, int conditionObj7, int[] inPixels, int conditionObj8, int width, float cosAngle, int conditionObj2, int conditionObj3, int[] outPixels, int height, float sinAngle) {
        int sx = (randomGenerator.nextInt() & 0x7fffffff) % width;
        int sy = (randomGenerator.nextInt() & 0x7fffffff) % height;
        int rgb = inPixels[sy * width + sx];
        ...
        x = x0;
        y = y0;
        if (x < width && x >= conditionObj8 && y < height && y >= conditionObj7) {
            int rgb2 = background ? 0xffffffff : outPixels[y * width + x];
            outPixels[y * width + x] = ImageMath.mixColors(mix, rgb2, rgb);
        }
        if (Math.abs(dx) > Math.abs(dy)) {
            ...
            while (x != x1) {
                ...
                x += ddx;
                if (x < width && x >= conditionObj5 && y < height && y >= 0) {
                    int rgb2 = background ? 0xffffffff : outPixels[y * width + x];
                    outPixels[y * width + x] = ImageMath.mixColors(mix, rgb2, rgb);
                }
            }
            ...
            while (y != y1) {
                ...
                y += ddy;
                if (x < width && x >= conditionObj3 && y < height && y >= conditionObj2) {
                    int rgb2 = background ? 0xffffffff : outPixels[y * width + x];
                    outPixels[y * width + x] = ImageMath.mixColors(mix, rgb2, rgb);
                }
            }
