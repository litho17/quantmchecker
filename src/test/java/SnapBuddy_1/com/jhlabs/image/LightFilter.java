...
import SnapBuddy_1.com.jhlabs.vecmath.*;
import java.awt.*;
import java.io.*;
import java.util.*;
...
public class LightFilter extends WholeImageFilter {
    ...
    public void setViewDistance(float viewDistance) {
        setViewDistanceHelper(viewDistance);
    }
    ...
    public void setEnvironmentMap(BufferedImage environmentMap) {
        ...
        if (environmentMap != null) {
            setEnvironmentMapHelper(environmentMap);
        } else {
    ...
    public void setDiffuseColor(int diffuseColor) {
        setDiffuseColorHelper(diffuseColor);
    }
...
*/
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (normal.z >= conditionObj2) {
                    ...
                    if (reflectivity != conditionObj3 && environmentMap != null) {
                        filterPixelsHelper(position, envColor, areflectivity, inPixels, normal, height, width, diffuseColor, viewpoint, reflectivity);
                    }
                    // Shade the pixel
    ...
    public static class Light implements Cloneable {
        ...
        private void setDistanceHelper(float distance) {
            this.distance = distance;
        }

        private void setConeAngleHelper(float coneAngle) {
            this.coneAngle = coneAngle;
        }

        private void setCentreXHelper(float x) {
            centreX = x;
        }
    }
    ...
    private void setViewDistanceHelper(float viewDistance) {
        this.viewDistance = viewDistance;
    }

    private void setEnvironmentMapHelper(BufferedImage environmentMap) {
    ...
    private void setDiffuseColorHelper(int diffuseColor) {
        material.diffuseColor = diffuseColor;
    }

    private void addLightHelper(Light light) {
        lights.addElement(light);
    }

    private void filterPixelsHelper(Vector3f position, Color4f envColor, float areflectivity, int[] inPixels, Vector3f normal, int height, int width, Color4f diffuseColor, Vector3f viewpoint, float reflectivity) {
