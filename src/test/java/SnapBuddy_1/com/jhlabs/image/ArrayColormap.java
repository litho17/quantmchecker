...
*/
    ...
    public void setColorInterpolated(int index, int firstIndex, int lastIndex, int color) {
        setColorInterpolatedHelper(lastIndex, index, color, firstIndex);
    }
    ...
    public void setColorRange(int firstIndex, int lastIndex, int color1, int color2) {
        for (int i = firstIndex; i <= lastIndex; i++) map[i] = ImageMath.mixColors((float) (i - firstIndex) / (lastIndex - firstIndex), color1, color2);
    }
    ...
    public void setColorRange(int firstIndex, int lastIndex, int color) {
        for (int i = firstIndex; i <= lastIndex; i++) map[i] = color;
    }
    ...
    private void setColorInterpolatedHelper(int lastIndex, int index, int color, int firstIndex) {
        int firstColor = map[firstIndex];
        int lastColor = map[lastIndex];
        for (int i = firstIndex; i <= index; i++) map[i] = ImageMath.mixColors((float) (i - firstIndex) / (index - firstIndex), firstColor, color);
        for (int i = index; i < lastIndex; i++) map[i] = ImageMath.mixColors((float) (i - index) / (lastIndex - index), color, lastColor);
    }
}
