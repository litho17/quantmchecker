...
public class CellularFilter extends WholeImageFilter implements Function2D, Cloneable {
    ...
    public void setDistancePower(float distancePower) {
        setDistancePowerHelper(distancePower);
    }
        ...
        for (int i = 0; i < numPoints; i++) {
            ...
            switch(gridType) {
                ...
                case OCTAGONAL:
                    ...
                    if (randomness != conditionObj6) {
                        px += randomness * Noise.noise2(271 * (cubeX + px), 271 * (cubeY + py));
                        py += randomness * Noise.noise2(271 * (cubeX + px) + 89, 271 * (cubeY + py) + 137);
                    }
                    break;
                case TRIANGULAR:
                    if ((cubeY & 1) == conditionObj3) {
                        if (i == conditionObj5) {
                            px = 0.25f;
                            py = 0.35f;
                        } else {
            ...
            if (d < results[0].distance) {
                checkCubeHelper(d, dx, dy, cubeY, results, px, cubeX, py);
                ...
                checkCubeHelper1(d, dx, dy, cubeY, results, px, cubeX, py);
            }
        }
    ...
    public float evaluate(float x, float y) {
        ...
        float t = 0;
        int conditionObj10 = 3;
        for (int i = 0; i < conditionObj10; i++) t += coefficients[i] * results[i].distance;
        int conditionObj11 = 0;
        int conditionObj12 = 0;
        if (angleCoefficient != conditionObj11) {
            float angle = (float) Math.atan2(y - results[0].y, x - results[0].x);
            if (angle < conditionObj12)
                angle += 2 * (float) Math.PI;
            angle /= 4 * (float) Math.PI;
            t += angleCoefficient * angle;
        }
        int conditionObj13 = 0;
        if (gradientCoefficient != conditionObj13) {
    ...
    public float turbulence2(float x, float y, float freq) {
        float t = 0.0f;
        for (float f = 1.0f; f <= freq; f *= 2) t += evaluate(f * x, f * y) / f;
        return t;
    }
    ...
    private void setF1Helper(float v) {
        coefficients[0] = v;
    }

    private void setDistancePowerHelper(float distancePower) {
        this.distancePower = distancePower;
    }

    private void setAmountHelper(float amount) {
        this.amount = amount;
    }

    private void checkCubeHelper(float d, float dx, float dy, int cubeY, Point[] results, float px, int cubeX, float py) {
        Point p = results[2];
        results[2] = results[1];
        results[1] = results[0];
        results[0] = p;
    ...
    private void checkCubeHelper1(float d, float dx, float dy, int cubeY, Point[] results, float px, int cubeX, float py) {
        Point p = results[2];
        p.distance = d;
        p.dx = dx;
        p.dy = dy;
