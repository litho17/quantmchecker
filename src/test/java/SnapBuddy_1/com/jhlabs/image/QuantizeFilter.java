...
public class QuantizeFilter extends WholeImageFilter {
        ...
        if (!dither) {
            ...
            for (int y = 0; y < height; y++) {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    int eg = g1 - g2;
                    int eb = b1 - b2;
                    for (int i = -1; i <= conditionObj0; i++) {
                        int iy = i + y;
                        if (0 <= iy && iy < height) {
                            for (int j = -1; j <= conditionObj1; j++) {
                                int jx = j + x;
                                if (0 <= jx && jx < width) {
                                    int w;
                                    if (reverse)
    ...
    private void setNumColorsHelper(int numColors) {
        this.numColors = Math.min(Math.max(numColors, 8), 256);
    }

    private void setDitherHelper(boolean dither) {
        this.dither = dither;
    }
}
