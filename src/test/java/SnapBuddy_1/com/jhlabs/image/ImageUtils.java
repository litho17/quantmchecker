...
import java.util.*;
import SnapBuddy_1.com.jhlabs.image.*;
...
public abstract class ImageUtils {
    ...
    public static BufferedImage createImage(ImageProducer producer) {
        ...
        int conditionObj0 = 0;
        if ((pg.status() & ImageObserver.ABORT) != conditionObj0)
            throw new  RuntimeException("Image fetch aborted");
        int conditionObj1 = 0;
        if ((pg.status() & ImageObserver.ERROR) != conditionObj1)
    ...
    public static void paintCheckedBackground(Component c, Graphics g, int x, int y, int width, int height) {
        int conditionObj2 = 64;
        int conditionObj3 = 64;
        int conditionObj4 = 0;
        if (backgroundImage == null) {
            backgroundImage = new  BufferedImage(64, 64, BufferedImage.TYPE_INT_ARGB);
            Graphics bg = backgroundImage.createGraphics();
            for (int by = 0; by < conditionObj2; by += 8) {
                for (int bx = 0; bx < conditionObj3; bx += 8) {
                    bg.setColor(((bx ^ by) & 8) != conditionObj4 ? Color.lightGray : Color.white);
    ...
    public static Rectangle getSelectedBounds(BufferedImage p) {
        ...
        int conditionObj6 = 0;
        int conditionObj7 = 0;
        for (y1 = height - 1; y1 >= conditionObj5; y1--) {
            pixels = getRGB(p, 0, y1, width, 1, pixels);
            for (int x = 0; x < minX; x++) {
                if ((pixels[x] & 0xff000000) != conditionObj7) {
                    minX = x;
                    maxY = y1;
                    anySelected = true;
                    break;
            ...
            for (int x = width - 1; x >= maxX; x--) {
                if ((pixels[x] & 0xff000000) != conditionObj6) {
                    maxX = x;
                    maxY = y1;
                    anySelected = true;
                    break;
        ...
        for (int y = 0; y < y1; y++) {
            ...
            for (int x = 0; x < minX; x++) {
                if ((pixels[x] & 0xff000000) != conditionObj9) {
                    minX = x;
                    if (y < minY)
                        minY = y;
                    anySelected = true;
            ...
            for (int x = width - 1; x >= maxX; x--) {
                if ((pixels[x] & 0xff000000) != conditionObj8) {
                    maxX = x;
                    if (y < minY)
                        minY = y;
                    anySelected = true;
    ...
    public static void setRGB(BufferedImage image, int x, int y, int width, int height, int[] pixels) {
        setRGBHelper(height, width, image, pixels, y, x);
    }

    private static void setRGBHelper(int height, int width, BufferedImage image, int[] pixels, int y, int x) {
        int type = image.getType();
        if (type == BufferedImage.TYPE_INT_ARGB || type == BufferedImage.TYPE_INT_RGB)
