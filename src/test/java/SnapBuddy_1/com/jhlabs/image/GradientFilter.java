...
public class GradientFilter extends AbstractBufferedImageOp {
        ...
        for (int y = 0; y < h; y++) {
            ...
            if (colrel <= 0.0) {
                ...
                do {
                    ...
                    off++;
                    colrel += dx;
                } while (--j > conditionObj3 && colrel <= 0.0);
            }
            while (colrel < 1.0 && --j >= conditionObj2) {
                if (type == BILINEAR)
                    rgb = colormap.getColor(map(ImageMath.triangle(colrel)));
                else
    ...
    private void radialGradient(int[] pixels, int y, int w, int h) {
        radialGradientHelper(w, pixels, y);
    }
    ...
    private void conicalGradient(int[] pixels, int y, int w, int h) {
        conicalGradientHelper(w, pixels, y);
    }
    ...
    private void radialGradientHelper(int w, int[] pixels, int y) {
        int off = 0;
    ...
    private void conicalGradientHelper(int w, int[] pixels, int y) {
        int off = 0;
