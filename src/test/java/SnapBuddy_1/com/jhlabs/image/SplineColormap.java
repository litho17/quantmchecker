...
public class SplineColormap extends ArrayColormap {
    ...
    private void setKnotHelper(int color, int n) {
        yKnots[n] = color;
        rebuildGradient();
    }

    private void addKnotHelper(int color, int x) {
    ...
    private void rebuildGradientHelper() {
        xKnots[0] = -1;
        xKnots[numKnots - 1] = 256;
        yKnots[0] = yKnots[1];
        yKnots[numKnots - 1] = yKnots[numKnots - 2];
        int conditionObj1 = 256;
        for (int i = 0; i < conditionObj1; i++) map[i] = ImageMath.colorSpline(i, numKnots, xKnots, yKnots);
    }

    private void sortKnotsHelper(int i) {
        for (int j = 1; j < i; j++) {
