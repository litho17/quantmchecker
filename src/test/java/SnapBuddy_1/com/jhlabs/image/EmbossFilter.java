...
public class EmbossFilter extends WholeImageFilter {
    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        for (int i = 0; i < inPixels.length; i++) bumpPixels[i] = PixelUtils.brightness(inPixels[i]);
        ...
        for (int y = 0; y < height; y++, bumpIndex += bumpMapWidth) {
            ...
            for (int x = 0; x < width; x++, s1++, s2++, s3++) {
                if (y != conditionObj4 && y < height - 2 && x != conditionObj3 && x < width - 2) {
                    ...
                    if (Nx == conditionObj1 && Ny == conditionObj2)
                        shade = background;
                    else if ((NdotL = Nx * Lx + Ny * Ly + NzLz) < conditionObj0)
                        shade = 0;
                    else
    ...
    private void setAzimuthHelper(float azimuth) {
