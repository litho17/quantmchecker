...
public class HalftoneFilter extends AbstractBufferedImageOp {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                filterHelper(inPixels, s, maskPixels, maskWidth, x);
            }
    ...
    private void setSoftnessHelper(float softness) {
    ...
    private void filterHelper(int[] inPixels, float s, int[] maskPixels, int maskWidth, int x) {
