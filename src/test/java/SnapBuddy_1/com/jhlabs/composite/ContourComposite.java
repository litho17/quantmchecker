...
class ContourCompositeContext implements CompositeContext {
    ...
    public void compose(Raster src, Raster dstIn, WritableRaster dstOut) {
        composeHelper(dstIn, src, dstOut);
    }

    private void composeHelper(Raster dstIn, Raster src, WritableRaster dstOut) {
        int x = src.getMinX();
        int y = src.getMinY();
        int w = src.getWidth();
        int h = src.getHeight();
        ...
        for (int i = 0; i < h; i++) {
            ...
            for (int j = 0; j < w; j++) {
                ...
                int alphaAbove = i != conditionObj5 ? srcPix2[k + 3] : alpha;
                if (i != conditionObj3 && j != conditionObj4 && ((alpha ^ lastAlpha) & 0x80) != conditionObj2 || ((alpha ^ alphaAbove) & 0x80) != conditionObj1) {
                    if ((offset + i + j) % 10 > conditionObj0) {
                        dstOutPix[k] = 0x00;
                        dstOutPix[k + 1] = 0x00;
                        dstOutPix[k + 2] = 0x00;
                    } else {
