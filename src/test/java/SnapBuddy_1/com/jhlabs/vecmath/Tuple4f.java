...
public class Tuple4f {
    ...
    private void absoluteHelper1(Tuple4f t) {
        x = Math.abs(t.x);
        y = Math.abs(t.y);
        z = Math.abs(t.z);
        w = Math.abs(t.w);
    ...
    private void clampHelper(float min, float max) {
        if (x < min)
            x = min;
        else if (x > max)
            x = max;
    ...
    private void getHelper(float[] t) {
        t[0] = x;
        t[1] = y;
        t[2] = z;
        t[3] = w;
    ...
    private void negateHelper1(Tuple4f t) {
        x = -t.x;
        y = -t.y;
        z = -t.z;
        w = -t.w;
    ...
    private void scaleHelper(float s) {
        x *= s;
        y *= s;
        z *= s;
        w *= s;
