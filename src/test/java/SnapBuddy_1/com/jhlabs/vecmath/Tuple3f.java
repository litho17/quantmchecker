...
*/
package SnapBuddy_1.com.jhlabs.vecmath;
...
public class Tuple3f {
    ...
    private void setHelper(Tuple3f t) {
        x = t.x;
        y = t.y;
        z = t.z;
    }

    private void getHelper(Tuple3f t) {
        t.x = x;
        t.y = y;
        t.z = z;
    }

    private void interpolateHelper(Tuple3f t, float alpha) {
        float a = 1 - alpha;
        x = a * x + alpha * t.x;
        y = a * y + alpha * t.y;
        z = a * z + alpha * t.z;
    ...
    private void addHelper(Tuple3f t2, Tuple3f t1) {
        x = t1.x + t2.x;
        y = t1.y + t2.y;
        z = t1.z + t2.z;
    }

    private void subHelper(Tuple3f t) {
        x -= t.x;
        y -= t.y;
        z -= t.z;
    }

    private void subHelper1(Tuple3f t2, Tuple3f t1) {
        x = t1.x - t2.x;
        y = t1.y - t2.y;
        z = t1.z - t2.z;
    }

    private void scaleAddHelper(Tuple3f t, float s) {
        x += s * t.x;
        y += s * t.y;
        z += s * t.z;
    }
