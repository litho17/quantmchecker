...
public class DES {
    ...
    long key) {
        ...
        int conditionObj3 = 16;
        int conditionObj4 = 1;
        // subkey.
        for (int i = 0; i < conditionObj3; i++) {
            // rotate the 28-bit values
            if (rotations[i] == conditionObj4) {
                // rotate by 1 bit
    ...
    long key) {
        ...
        int conditionObj5 = 16;
        // perform 16 rounds
        for (int i = 0; i < conditionObj5; i++) {
            int previous_l = l;
    ...
    public static byte[] encrypt(byte[] message, byte[] key) {
        ...
        for (int i = 0; i < message.length; i += 8) {
            encryptHelper(message, ciphertext, key, i);
        }
        return ciphertext;
    }
    ...
    private static byte[] passwordToKey(String password) {
        ...
        int conditionObj6 = 8;
        int conditionObj7 = 8;
        for (int i = 0; i < conditionObj6; i++) {
            if (i < pwbytes.length) {
                byte b = pwbytes[i];
                // flip the byte
                byte b2 = 0;
                for (int j = 0; j < conditionObj7; j++) {
                    b2 <<= 1;
                    b2 |= (b & 0x01);
                    b >>>= 1;
                }
    ...
    private static void encryptHelper(byte[] message, byte[] ciphertext, byte[] key, int i) {
        encryptBlock(message, i, ciphertext, i, key);
    }

    private static void passwordToKeyHelper(byte[] key, int i) {
        key[i] = 0;
    }
