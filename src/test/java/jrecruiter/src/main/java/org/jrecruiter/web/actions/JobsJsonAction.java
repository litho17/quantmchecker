...
public class JobsJsonAction extends BaseAction {
	...
	public String execute() {
		...
		@Inv("= entries 0") final List<SyndEntry> entries = CollectionUtils.getArrayList();

		@Bound("+ jobService.jobs 1") int i;
		@Inv("= sortOrders c70") final Map<String, String> sortOrders = CollectionUtils.getHashMap();
		c70: sortOrders.put("updateDate", "DESC"); //FIXME

		@Inv("(= (- jsonJobs it) (- c89 c77))") List<JSONObject> jsonJobs = new ArrayList<JSONObject>(jobService.getJobs(20, 1, sortOrders, null).size());

		@Iter("<= it jobService.jobs") Iterator<Job> it = jobService.getJobs(20, 1, sortOrders, null).iterator();
		while (it.hasNext()) {
			Job job;
			c77: job = it.next();
			try {
