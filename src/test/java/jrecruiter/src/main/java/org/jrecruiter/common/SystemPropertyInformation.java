...
public enum SystemPropertyInformation {
	...
	public static List<SystemPropertyInformation> getSystemPropertyValuesAsList() {
		...
		@Bound("SystemPropertyInformation") int j;
		@Inv("(= (- systemPropertyValues i) (- c94 c95))") final List<SystemPropertyInformation>systemPropertyValues = CollectionUtils.getArrayList();

		SystemPropertyInformation systemProperty;
		@Iter("<= i SystemPropertyInformation") int i = 0;
		for (; i < SystemPropertyInformation.values().length;) {
			systemProperty = SystemPropertyInformation.values()[i];
			c94: systemPropertyValues.add(systemProperty);
			c95: i = i + 1;
		}
