...
package tomcat.org.apache.jasper.compiler;
...
import tomcat.javax.el.MethodExpression;
import tomcat.javax.el.ValueExpression;
import tomcat.javax.servlet.jsp.tagext.TagAttributeInfo;
import tomcat.javax.servlet.jsp.tagext.TagInfo;
import tomcat.javax.servlet.jsp.tagext.TagVariableInfo;
import tomcat.javax.servlet.jsp.tagext.VariableInfo;

import tomcat.org.apache.jasper.Constants;
import tomcat.org.apache.jasper.JasperException;
import tomcat.org.apache.jasper.JspCompilationContext;
import tomcat.org.apache.jasper.compiler.Node.NamedAttribute;
import tomcat.org.apache.jasper.runtime.JspRuntimeLibrary;
...
class Generator {
    ...
    private static final String VAR_EXPRESSIONFACTORY =
        System.getProperty("tomcat.org.apache.jasper.compiler.Generator.VAR_EXPRESSIONFACTORY", "_el_expressionfactory");
    private static final String VAR_INSTANCEMANAGER =
        System.getProperty("tomcat.org.apache.jasper.compiler.Generator.VAR_INSTANCEMANAGER", "_jsp_instancemanager");
    private static final boolean POOL_TAGS_WITH_EXTENDS =
        Boolean.getBoolean("tomcat.org.apache.jasper.compiler.Generator.POOL_TAGS_WITH_EXTENDS");
    ...
    private static final boolean STRICT_GET_PROPERTY = Boolean.parseBoolean(
            System.getProperty(
                    "tomcat.org.apache.jasper.compiler.Generator.STRICT_GET_PROPERTY",
                    "true"));
    ...
    private void generateGetters() {
        out.printil("public ExpressionFactory _jsp_getExpressionFactory() {");
        out.pushIndent();
        if (!ctxt.isTagFile()) {
            out.printin("if (");
        ...
        out.printil("public tomcat.org.apache.tomcat.InstanceManager _jsp_getInstanceManager() {");
        out.pushIndent();
        if (!ctxt.isTagFile()) {
            out.printin("if (");
            ...
            out.println(" = tomcat.org.apache.jasper.runtime.InstanceManagerFactory.getInstanceManager(getServletConfig());");
            out.popIndent();
            out.printil("}");
            out.popIndent();
            out.printil("}");
    ...
    private void generateInit() {
        ...
        if (isPoolingEnabled) {
            for (int i = 0; i < tagHandlerPoolNames.size(); i++) {
                ...
                out.print(" = tomcat.org.apache.jasper.runtime.TagHandlerPool.getTagHandlerPool(");
                if (ctxt.isTagFile()) {
                    out.print("config");
                } else {
        ...
        if (ctxt.isTagFile()) {
            ...
            out.printin(VAR_INSTANCEMANAGER);
            out.println(" = tomcat.org.apache.jasper.runtime.InstanceManagerFactory.getInstanceManager(config);");
        }
    ...
    private void genPreambleStaticInitializers() {
        ...
        out.printil("        JspFactory.getDefaultFactory();");
        out.println();
    ...
    private void genPreambleClassVariableDeclarations() {
        if (isPoolingEnabled && !tagHandlerPoolNames.isEmpty()) {
            for (int i = 0; i < tagHandlerPoolNames.size(); i++) {
                out.printil("private tomcat.org.apache.jasper.runtime.TagHandlerPool "
                        + tagHandlerPoolNames.elementAt(i) + ";");
            }
            out.println();
        }
        out.printin("private volatile ExpressionFactory ");
        out.print(VAR_EXPRESSIONFACTORY);
        out.println(";");
        out.printin("private volatile tomcat.org.apache.tomcat.InstanceManager ");
        out.print(VAR_INSTANCEMANAGER);
        out.println(";");
        out.println();
    }
    ...
    private void generatePreamble(Node.Nodes page) throws JasperException {
        ...
        out.printin("    implements tomcat.org.apache.jasper.runtime.JspSourceDependent,");
        out.println();
        out.printin("                 tomcat.org.apache.jasper.runtime.JspSourceImports");
        ...
        out.pushIndent();
        out.pushIndent();
        out.printil("throws java.io.IOException, ServletException {");
        out.popIndent();
        out.println();

        // Method check
        if (!pageInfo.isErrorPage()) {
            out.printil("if (!DispatcherType.ERROR.equals(request.getDispatcherType())) {");
            out.pushIndent();
        ...
        if (pageInfo.isErrorPage()) {
            out.printil("java.lang.Throwable exception = tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.getThrowable(request);");
    ...
    private class GenerateVisitor extends Node.Visitor {
        ...
        private String attributeValue(Node.JspAttribute attr, boolean encode,
            ...
            if (attr.isExpression()) {
                if (encode) {
                    return "tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.URLEncode(String.valueOf("
                ...
                if (encode) {
                    return "tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.URLEncode("
                            + v + ", request.getCharacterEncoding())";
                }
                return v;
        ...
        private void printParams(Node n, String pageParam, boolean literal)
            ...
            class ParamVisitor extends Node.Visitor {
                ...
                public void visit(Node.ParamAction n) throws JasperException {
                    ...
                    out.print(separator);
                    out.print(" + ");
                    out.print("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary."
        ...
        public void visit(Node.IncludeAction n) throws JasperException {
            ...
            out.printin("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.include(request, response, "
                    + pageParam);
        ...
        public void visit(Node.GetProperty n) throws JasperException {
            ...
            if (beanInfo.checkVariable(name)) {
                ...
                out.printil("out.write(tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.toString("
                        + "((("
                        + beanName
                ...
                out.printil("out.write(tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.toString"
                        + "(tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.handleGetProperty"
                        + "(_jspx_page_context.findAttribute(\""
                        + name
                        + "\"), \""
                        + property
        ...
        public void visit(Node.SetProperty n) throws JasperException {
            ...
            if ("*".equals(property)) {
                out.printil("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.introspect("
                        + "_jspx_page_context.findAttribute("
                        + "\""
                        + name + "\"), request);");
            } else if (value == null) {
                ...
                out.printil("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.introspecthelper("
                        + "_jspx_page_context.findAttribute(\""
                        + name
                        + "\"), \""
                        + property
                ...
                out.printil("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.handleSetProperty("
                        + "_jspx_page_context.findAttribute(\""
                        + name
                        + "\"), \"" + property + "\",");
                ...
                out.printil("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.handleSetPropertyExpression("
                        + "_jspx_page_context.findAttribute(\""
                        + name
                        + "\"), \""
                        + property
                ...
                out.printin("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.introspecthelper("
                        + "_jspx_page_context.findAttribute(\""
                        + name
                        + "\"), \"" + property + "\", ");
                out.print(attributeValue(value, false, null));
        ...
        public void visit(Node.InvokeAction n) throws JasperException {
            ...
            // page
            out.printil("((tomcat.org.apache.jasper.runtime.JspContextWrapper) this.jspContext).syncBeforeInvoke();");
        ...
        private void generateCustomStart(Node.CustomTag n,
            ...
            if (!n.hasEmptyBody()) {
                ...
                if (n.implementsBodyTag()) {
                    out.printin("if (");
                    out.print(tagEvalVar);
                    out.println(" != Tag.EVAL_BODY_INCLUDE) {");
                    ...
                    out.printin("out = tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.startBufferedBody(");
                    out.print("_jspx_page_context, ");
                    out.print(tagHandlerVar);
                    out.println(");");
                    out.popIndent();
        ...
        private void generateCustomEnd(Node.CustomTag n, String tagHandlerVar,
            ...
            if (!n.hasEmptyBody()) {
                if (n.implementsIterationTag()) {
                    ...
                    out.printil("if (evalDoAfterBody != BodyTag.EVAL_BODY_AGAIN)");
                    out.pushIndent();
                    out.printil("break;");
                    out.popIndent();
            ...
            out.printin("if (");
            out.print(tagHandlerVar);
            out.println(".doEndTag() == Tag.SKIP_PAGE) {");
            out.pushIndent();
            if (isTagFile || isFragment) {
                out.printil("throw new SkipPageException();");
            } else {
            ...
            out.printil("} finally {");
            out.pushIndent();
            out.printin("tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.releaseTag(");
            out.print(tagHandlerVar);
        ...
        private String evaluateAttribute(TagHandlerInfo handlerInfo,
            ...
            if (attr.isExpression()) {
                ...
                if (attr.getEL() != null && attr.getEL().getMapName() != null) {
                    sb.setLength(0);
                    sb.append("new tomcat.org.apache.jasper.el.ELContextWrapper(");
                    sb.append(elContext);
                    sb.append(',');
                ...
                if (attr.isDeferredInput()
                    ...
                    sb.append("new tomcat.org.apache.jasper.el.JspValueExpression(");
                    ...
                    sb.append("new tomcat.org.apache.jasper.el.JspMethodExpression(");
                    sb.append(quote(mark));
        ...
        private void generateSetters(Node.CustomTag n, String tagHandlerVar,
            ...
            if (isTagFile && parent == null) {
                out.printin(tagHandlerVar);
                out.print(".setParent(");
                out.print("new TagAdapter(");
                out.print("(SimpleTag) this ));");
            } else if (!simpleTag) {
                out.printin(tagHandlerVar);
                out.print(".setParent(");
                if (parent != null) {
        ...
        private String convertString(Class<?> c, String s, String attrName,
            ...
            if (propEditorClass != null) {
                ...
                return "("
                        + className
                        + ")tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.getValueFromBeanInfoPropertyEditor("
                ...
                return "("
                        + className
                        + ")tomcat.org.apache.jasper.runtime.JspRuntimeLibrary.getValueFromPropertyEditorManager("
                        + className + ".class, \"" + attrName + "\", " + quoted
                        + ")";
            }
        }
        ...
        private void generateJspFragment(Node n, String tagHandlerVar)
            ...
            // the tomcat.org.apache.jasper.runtime package as a support class.
        ...
        public String generateNamedAttributeJspFragment(Node.NamedAttribute n,
            ...
            out.printin("JspFragment " + varName
                    + " = ");
            generateJspFragment(n, tagHandlerVar);
            out.println(";");
    ...
    private void generateTagHandlerPreamble(JasperTagInfo tagInfo,
        ...
        out.printil("    extends SimpleTagSupport");
        out.printin("    implements tomcat.org.apache.jasper.runtime.JspSourceDependent,");
        out.println();
        out.printin("                 tomcat.org.apache.jasper.runtime.JspSourceImports");
        if (tagInfo.hasDynamicAttributes()) {
            out.println(",");
            out.printin("               DynamicAttributes");
        }
        out.println(" {");
        out.pushIndent();
    ...
    private void generateTagHandlerPostamble(TagInfo tagInfo) {
        ...
        out.printil("((tomcat.org.apache.jasper.runtime.JspContextWrapper) jspContext).syncEndTagFile();");
        if (isPoolingEnabled && !tagHandlerPoolNames.isEmpty()) {
    ...
    private void generateSetJspContext(TagInfo tagInfo) {
        ...
        if (aliasSeen) {
            out.printil("this.jspContext = new tomcat.org.apache.jasper.runtime.JspContextWrapper(this, ctx, _jspx_nested, _jspx_at_begin, _jspx_at_end, aliasMap);");
        } else {
            out.printil("this.jspContext = new tomcat.org.apache.jasper.runtime.JspContextWrapper(this, ctx, _jspx_nested, _jspx_at_begin, _jspx_at_end, null);");
        }
        out.popIndent();
        out.printil("}");
        out.println();
     ...
     * DynamicAttributes.setDynamicAttribute() method,
     * which saves each dynamic attribute that is passed in so that a scoped
     * variable can later be created for it.
     */
    public void generateSetDynamicAttribute() {
        out.printil("public void setDynamicAttribute(java.lang.String uri, java.lang.String localName, java.lang.Object value) throws JspException {");
        out.pushIndent();
        /*
        ...
        for (int i = 0; i < attrInfos.length; i++) {
            ...
            if (attrInfos[i].isDeferredValue() || attrInfos[i].isDeferredMethod()) {
                ...
                if (!variableMapperVar) {
                    out.printil("VariableMapper _el_variablemapper = jspContext.getELContext().getVariableMapper();");
                    variableMapperVar = true;
                }

                out.printin("ValueExpression _el_ve");
                out.print(i);
                ...
                if (attrInfos[i].isDeferredMethod()) {
                    ...
                    out.print(",MethodExpression.class)");
                } else {
                    out.print(toGetterMethod(attrName));
                }
                out.println(");");
    ...
    private static class FragmentHelperClass {
        ...
        public void generatePreamble() {
            ...
            out.printil("    extends "
                    + "tomcat.org.apache.jasper.runtime.JspFragmentHelper");
            out.printil("{");
            out.pushIndent();
            out.printil("private JspTag _jspx_parent;");
        ...
        public void generatePostamble() {
            ...
            out.printil("try {");
            out.pushIndent();
            out.printil("Object _jspx_saved_JspContext = this.jspContext.getELContext().getContext(JspContext.class);");
            ...
            // restore nested JspContext on ELContext
            out.printil("jspContext.getELContext().putContext(JspContext.class,_jspx_saved_JspContext);");
