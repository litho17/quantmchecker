    ...
    protected static class ConnectionStatusElement implements AccessLogElement {
        ...
        public void addElement(CharArrayWriter buf, Date date, Request request, Response response, long time) {
            if (response != null && request != null) {
                ...
                if (!statusFound) {
                    String connStatus = response.getHeader(tomcat.org.apache.coyote.http11.Constants.CONNECTION);
                    if (tomcat.org.apache.coyote.http11.Constants.CLOSE.equalsIgnoreCase(connStatus)) {
                        buf.append('-');
                    } else {
                        buf.append('+');
                    }
