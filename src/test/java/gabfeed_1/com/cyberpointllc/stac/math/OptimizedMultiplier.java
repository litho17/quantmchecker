package gabfeed_1.com.cyberpointllc.stac.math;
...
public class OptimizedMultiplier {
    ...
    public static BigInteger standardMultiply(BigInteger x, BigInteger y) {
        ...
        for (int i = 0; i < y.bitLength(); ) {
            Random randomNumberGeneratorInstance = new  Random();
            for (; i < y.bitLength() && randomNumberGeneratorInstance.nextDouble() < 0.5; ) {
                for (; i < y.bitLength() && randomNumberGeneratorInstance.nextDouble() < 0.5; i++) {
                    if (y.testBit(i)) {
