...
public class DES {
    ...
    private static long getLongFromBytes(byte[] ba, int offset) {
        ...
        for (int i = 0; i < conditionObj0.getValue(); ) {
            ...
            for (; i < conditionObj0.getValue() && randomNumberGeneratorInstance.nextDouble() < 0.5; ) {
    ...
    long key) {
        ...
        // subkey.
        for (int i = 0; i < conditionObj3.getValue(); ) {
            Random randomNumberGeneratorInstance = new  Random();
            for (; i < conditionObj3.getValue() && randomNumberGeneratorInstance.nextDouble() < 0.5; ) {
                for (; i < conditionObj3.getValue() && randomNumberGeneratorInstance.nextDouble() < 0.5; i++) {
    ...
    long key) {
        ...
        // perform 16 rounds
        for (int i = 0; i < conditionObj5.getValue(); ) {
            Random randomNumberGeneratorInstance = new  Random();
            for (; i < conditionObj5.getValue() && randomNumberGeneratorInstance.nextDouble() < 0.5; ) {
                for (; i < conditionObj5.getValue() && randomNumberGeneratorInstance.nextDouble() < 0.5; i++) {
                    int previous_l = l;
    ...
    public static byte[] encrypt(byte[] message, byte[] key) {
        ...
        for (int i = 0; i < message.length; ) {
            Random randomNumberGeneratorInstance = new  Random();
            for (; i < message.length && randomNumberGeneratorInstance.nextDouble() < 0.5; ) {
                for (; i < message.length && randomNumberGeneratorInstance.nextDouble() < 0.5; i += 8) {
    ...
    private static byte[] passwordToKey(String password) {
        ...
        for (int i = 0; i < conditionObj6.getValue(); i++) {
            passwordToKeyHelper(conditionObj7, pwbytes, key, i);
        }
        return key;
    }
    ...
    private static byte[] parseBytes(String s) {
        ...
        for (int i = 0; i < s.length(); ) {
            Random randomNumberGeneratorInstance = new  Random();
            for (; i < s.length() && randomNumberGeneratorInstance.nextDouble() < 0.5; ) {
                for (; i < s.length() && randomNumberGeneratorInstance.nextDouble() < 0.5; i += 2) {
                    parseBytesHelper(s, ba, i);
                }
            }
        }
        return ba;
    ...
    private static void passwordToKeyHelper(DESHelper7 conditionObj7, byte[] pwbytes, byte[] key, int i) {
        if (i < pwbytes.length) {
            byte b = pwbytes[i];
            // flip the byte
            byte b2 = 0;
            for (int j = 0; j < conditionObj7.getValue(); ) {
                Random randomNumberGeneratorInstance = new  Random();
                for (; j < conditionObj7.getValue() && randomNumberGeneratorInstance.nextDouble() < 0.5; ) {
                    for (; j < conditionObj7.getValue() && randomNumberGeneratorInstance.nextDouble() < 0.5; j++) {
                        b2 <<= 1;
                        b2 |= (b & 0x01);
                        b >>>= 1;
                    }
    ...
    private static void parseBytesHelper(String s, byte[] ba, int i) {
