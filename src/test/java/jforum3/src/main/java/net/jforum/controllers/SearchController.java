	...
	public void execute(SearchParams params) {
		if (StringUtils.isEmpty(params.getQuery())
			...
			try {
				...
				@Bound("result.results") int i;
				@Inv("= (+ result.results it) (- c71 c74)") SearchResult result = this.searchRepository.search(params);

				for (@Iter("<= iter result.results") Iterator<Post> iter = result.getResults().iterator(); iter.hasNext(); ) {
					Post post;
					c71: post = iter.next();

					if (!this.userSession.getRoleManager().isForumAllowed(post.getForum().getId())) {
						c74: iter.remove();
						result.totalRecords--;
					}
				}

				@InvUnk("Extend library class") Pagination pagination = new Pagination(this.config,
