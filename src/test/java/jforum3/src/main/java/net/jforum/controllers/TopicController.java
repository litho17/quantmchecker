...
public class TopicController {
	...
	public void vote(int topicId, int pollId, int optionId) {
		...
		if (userSession.isLogged() && optionId != 0) {
			...
			if (!this.pollRepository.hasUserVoted(poll, this.userSession.getUser()) && poll.isOpen()) {

				PollVoter voter = new PollVoter();
				voter.setIp(userSession.getIp());
				voter.setPoll(poll);
				voter.setUser(this.userSession.getUser());
	...
	public void replySave(Topic topic, Post post, PostFormOptions options) {
		...
		@Inv("= (- attachments i) (- c323 c325)") List<AttachedFile> attachments = new ArrayList<AttachedFile>();
			...
			if (!StringUtils.isEmpty(t)) {
				...
				if (total >= 1) {
					...
					for (; i < total;) {
						...
						c323: attachments.add(new AttachedFile(attachment, uploadUtils));
	...
	public void list(int topicId, int page, boolean viewPollResults) {
		...
		@InvUnk("Extend library class") Pagination pagination = new Pagination(this.config, page).forTopic(topic);
		...
		@InvUnk("Unknown API") List<Post> posts = topic.getPosts(pagination.getStart(), pagination.getRecordsPerPage());
	...
	public void addSave(Topic topic, Post post, PostFormOptions postOptions, List<PollOption> pollOptions) {
		...
		@Inv("= (- attachments i) (- c461 c463)") List<AttachedFile> attachments = new ArrayList<AttachedFile>();
			...
			if (!StringUtils.isEmpty(t)) {
				...
				if (total >= 1) {
					...
					for (; i < total;) {
						...
						c461: attachments.add(new AttachedFile(attachment, uploadUtils));
						totalSize += attachment.getFilesize();
						c463: i = i + 1;
					}
				}
	...
	private void redirecToListing(Topic topic, Post post) {
		@InvUnk("Extend library class") Pagination pagination = new Pagination(this.config, 0).forTopic(topic);
		@Bound("2") int i;
		@Inv("= url (+ c429 c430)") StringBuilder url = new StringBuilder(
