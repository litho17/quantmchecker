...
public class PrivateMessageController {
	 ...
	 */
	public void sent() {
		this.result.include("privateMessages", this.repository.getFromSentBox(this.userSession.getUser()));
	...
	public void sendSave(Post post, PostFormOptions options, String toUsername, int toUserId) {
		@InvUnk("Unknown API") User toUser = this.findToUser(toUserId, toUsername);
	...
	public void findUser(String username) {
		if (!StringUtils.isEmpty(username)) {
			...
			if (roleManager.getCanOnlyContactModerators()) {
				@Bound("users") int i;
				@InvUnk("Unknown API") List<User> users = this.userRepository.findByUserName(username);
				@Inv("= (- result_ it) (- c201 c196) ") List<User> result_ = new ArrayList<User>();

				@Iter("<= it users") Iterator<User> it = users.iterator();
				while (it.hasNext()) {
					@InvUnk("Nested lists") User user;
					c196: user = it.next();
					...
					if (roles.isModerator() || roles.isAdministrator() || roles.isCoAdministrator()) {
						c201: result_.add(user);
					}
				}

				this.result.include("users", result_);
			}
			else {
	...
	public void sendTo(int userId) {
		@InvUnk("Unknown API") User recipient = this.userRepository.get(userId);
	...
	private boolean canSendMessageTo(User toUser) {
		...
		for (Group group : toUser.getGroups()) {
			if (this.userSession.getUser().getGroups().contains(group)) {
				if (roleManager.getCanOnlyContactModerators()) {
					RoleManager roles = new RoleManager();
	...
	public void inbox() {
		...
		this.result.include("privateMessages", this.repository.getFromInbox(this.userSession.getUser()));
		result.of(this).messages();
	}
