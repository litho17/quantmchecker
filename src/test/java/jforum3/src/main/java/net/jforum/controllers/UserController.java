...
public class UserController {
	...
	public void list(int page) {
		...
		if (!roleManager.isUserListingEnabled()) {
			...
			@InvUnk("Extend library class") Pagination pagination = new Pagination(this.config, page)
			...
			if (roleManager.roleExists(SecurityConstants.INTERACT_OTHER_GROUPS)) {
				...
				this.result.include("users", this.userRepository.getAllUsers(pagination.getStart(),
					pagination.getRecordsPerPage(), this.userSession.getUser().getGroups()));
			}
	 ...
	 */
	public void logout() {
		this.sessionManager.storeSession(userSession.getSessionId());

		userSession.becomeAnonymous(this.config.getInt(ConfigKeys.ANONYMOUS_USER_ID));

		this.sessionManager.remove(userSession.getSessionId());
	...
	/**
	 * Called from ??? to validate the user credentials
	 *
	...
	public void authenticateUser(String username,
		...
		String returnPath) {
		@InvUnk("Dynamic dispatch") User user = this.userService.validateLogin(username, password);
	...
	public void edit(int userId) {
		@InvUnk("Unknown API") User userToEdit = this.userRepository.get(userId);
	...
	public void editSave(User user, Integer avatarId, UploadedFile image, Integer rankingId) {
		@Bound("0") int i;
		@Inv("= avatar.users 0") Avatar avatar = null;
	...
	public void recoverPasswordValidate(String hash,
		...
		@InvUnk("Unknown API") User user = this.userRepository.validateLostPasswordHash(username, hash);
	...
	public void posts(int userId, int page) {
		@InvUnk("Unknown API") User user = userRepository.get(userId);

		@InvUnk("Extend library class") Pagination pagination = new Pagination(this.config, page).forUserPosts(user);

		@InvUnk("Unknown API") List<Post> posts = userRepository.getPosts(user, pagination.getStart(), pagination.getRecordsPerPage());
	...
	public void topics(int userId, int page) {
		...
		@InvUnk("Extend library class") Pagination pagination = new Pagination(this.config, page).forUserTopics(user, userRepository.getTotalTopics(userId));

		@InvUnk("Unknown API") List<Topic> topics = userRepository.getTopics(user, pagination.getStart(), pagination.getRecordsPerPage());
