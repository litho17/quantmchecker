...
public class RoleManager {
	@InvUnk("Complex loop") private Map<String, Role> roles = new HashMap<String, Role>();
	...
	public void setGroups(List<Group> groups) {
		...
		if (groups != null) {
			Iterator<Group> it1;
			c42: it1 = groups.iterator();
			while (it1.hasNext()) {
				Group group;
				c45: group = it1.next();
				@InvUnk("Nested lists") List<Role> currentGroupRoles = group.getRoles();
				Iterator<Role> it2;
				c48: it2 = currentGroupRoles.iterator();
				while (it2.hasNext()) {
					@InvUnk("Nested lists") Role role;
					c51: role = it2.next();

					if (this.roles.get(role.getName()) == null) {
						c55: this.roles.put(role.getName(), new Role(role));
					}
					else {
						...
						if (role.getName().equals(SecurityConstants.FORUM_READ_ONLY)
							...
							this.roles.get(role.getName()).getRoleValues().retainAll(role.getRoleValues());
						}
						else {
							this.roles.get(role.getName()).getRoleValues().addAll(role.getRoleValues());
						}
					}
				}
			}
	...
	private void readReplyOnlySecondPass(String roleName, List<Group> groups) {
		@InvUnk("Nested lists") Role role = this.roles.get(roleName);
	...
	public int[] getRoleValues(String name) {
		if (this.get(name) == null || this.get(name).getRoleValues().size() == 0) {
			return new int[0];
		}
		...
		int[] data = new int[this.get(name).getRoleValues().size()];
		int counter = 0;
		for (int value : this.get(name).getRoleValues()) {
			data[counter++] = value;
		}
	...
	public List<Integer> getRoleValuesAsList(String name) {
		return this.get(name) != null ? this.get(name).getRoleValues() : new ArrayList<Integer>();
	}
	...
	public boolean roleExists(String name, int value) {
		return this.get(name) != null && this.get(name).getRoleValues().contains(value);
	}
