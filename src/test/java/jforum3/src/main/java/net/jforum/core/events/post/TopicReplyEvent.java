...
public class TopicReplyEvent extends EmptyPostEvent {
	...
	public void added(Post post) {
		...
		if (this.config.getBoolean(ConfigKeys.MAIL_NOTIFY_ANSWERS)) {
			@InvUnk("Unknown API") List<User> users = this.watchRepository.getUsersWaitingNotification(post.getTopic());

			@InvUnk("Extend library class") Spammer spammer = this.spammerFactory.newTopicReply(post.getTopic(), users);
