...
public class SessionManager {
	 ...
	 */
	@Summary({"loggedSessions", "1", "anonymousSessions", "1"})
	public synchronized void add(UserSession userSession) {
		...
		if (!userSession.isBot()) {
			...
			if (userSession.getUser().getId() == this.config.getInt(ConfigKeys.ANONYMOUS_USER_ID)) {
				...
				if (this.isUserInSession(userSession.getUser().getId()) != null) {
					userSession.setLastVisit(this.isUserInSession(userSession.getUser().getId()).getLastVisit());
					this.remove(this.isUserInSession(userSession.getUser().getId()).getSessionId());
				}
				else {
	...
	public void computeAllOnlineModerators() {
		...
		@InvUnk("Unknown API") Collection<UserSession> sessions = loggedSessions.values();
	 ...
	 */
	@Summary({"this.loggedSessions", "-1", "this.anonymousSessions", "-1"})
	public synchronized void remove(String sessionId) {
		if (loggedSessions.containsKey(sessionId)) {

			if (this.getUserSession(sessionId).getRoleManager() != null
				&& this.getUserSession(sessionId).getRoleManager().isModerator() && this.moderatorsOnline > 0) {
				this.moderatorsOnline--;
			}
	...
	public List<UserSession> getAllSessions() {
		@Bound("+ loggedSessions anonymousSessions") int i;
		@Inv("= (- list it1 it2) (- (+ c162 c167) c161 c166)") List<UserSession> list = new ArrayList<UserSession>();
		@Iter("<= it1 loggedSessions") Iterator<UserSession> it1 = loggedSessions.values().iterator();
		UserSession us;
		while (it1.hasNext()) {
			c161: us = it1.next();
			c162: list.add(us);
		}
		@Iter("<= it2 anonymousSessions") Iterator<UserSession> it2 = anonymousSessions.values().iterator();
		while (it2.hasNext()) {
			c166: us = it2.next();
			c167: list.add(us);
		}
	...
	public UserSession getUserSession(String sessionId) {
		return anonymousSessions.get(sessionId) != null ? anonymousSessions.get(sessionId) : loggedSessions.get(sessionId);
	}
	...
	public UserSession refreshSession(UserSession userSession) {
		...
		if (userSession.getUser() == null || userSession.getUser().getId() == 0) {
			...
			@InvUnk("Unknown API") User anonymousUser = this.userRepository.get(userSession.getUser().getId());
	...
	public void storeSession(String sessionId) {
		if (this.getUserSession(sessionId) != null && this.getUserSession(sessionId).getUser().getId() != this.config.getInt(ConfigKeys.ANONYMOUS_USER_ID)) {
			Session session = this.getUserSession(sessionId).asSession();
	...
	private boolean checkAutoLogin(UserSession userSession) {
		...
		if (hashCookie != null && userIdCookie != null
			...
			@InvUnk("Unknown API") User user = this.userRepository.get(Integer.parseInt(userId));
	...
	private void checkSSO(UserSession userSession) {
		try {
			...
			if (StringUtils.isEmpty(username)) {
				...
				@Inv("and (= utils.user.posts 0) (= utils.user.groups 0) (= utils.user.extra 0) (= utils.user.avatar.users 0)") SSOUtils utils = new SSOUtils(this.userRepository);
				...
				if (!userExists) {
					...
					utils.user.setPassword(password);
					utils.user.setEmail(email);
					utils.user.setActive(true);

					utils.userRepository.add(utils.user);
				}

				logger.info(String.format("g: username=%s, jforumUserId=%s",
						utils.user != null ? utils.user.getUsername() : "returned null",
						utils.user != null ? utils.user.getId() : "returned null"));

				this.configureUserSession(userSession, utils.user);

				if (utils.user == null || utils.user.getId() == 0) {
