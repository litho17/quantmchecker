...
public class ModerationController {
	...
	public void showActivityLog(int page) {
		...
		@InvUnk("Unknown API") List<ModerationLog> logs = this.logRepository.getAll(
	...
	public void deleteTopics(int forumId, String returnUrl, int[] topicIds,
		...
		if (this.roleManager.getCanDeletePosts()) {
			@Bound("topicIds") int j;
			@Inv("= (- topics i) (- c185 c186)") List<Topic> topics = new ArrayList<Topic>();
			@Iter("<= i topicIds") int i = 0;
			for (; i < topicIds.length;) {
				int topicId;
				topicId = topicIds[i];
				Topic topic = this.topicRepository.get(topicId);
				c185: topics.add(topic);
				c186: i = i + 1;
			}
