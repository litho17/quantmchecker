...
public class AvatarService {
	...
	public void update(Avatar avatar, UploadedFile uploadedFile) {
		...
		@InvUnk("Unknown API") Avatar current = this.repository.get(avatar.getId());
	...
	public void delete(int... avatarId) {
		if (avatarId != null) {
			for (int id : avatarId) {
				@InvUnk("Unknown API") Avatar avatar = this.repository.get(id);
				this.delete(avatar);
			}
		}
	}
	...
	private String processImageUpload(Avatar avatar, UploadedFile uploadedFile) {
		...
		try {
			...
			@Inv("= ii.comments 0") ImageInfo ii = new ImageInfo();
		...
		catch (@InvUnk("Extend library class") ValidationException e) {
			file.delete();
			throw e;
		}
