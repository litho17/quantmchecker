...
public class WarShips {
    ...
    public Map<Square, Pin> fireTaken(String heightInMeters,
        ...
        return currentCompetition.fetchStrikeSquares(heightInMeters, speedInMetersPerSecond, elevationInDegrees, boardAngleInDegrees);
    }
    ...
    public void assignStage(Stage stage) {
        ...
        for (int p = 0; p < stage.commands.size(); p++) {
            String s = stage.commands.get(p);
    ...
    private void defineStrikeReportsTarget(List<Hit> strikes) {
        @Bound("strikes") int i;
        @Inv("= (- squaresStrike it) (- c291 c284)") Map<Square, Pin> squaresStrike = new HashMap<>();
        @Iter("<= it strikes") Iterator<Hit> it = strikes.iterator();
        while (it.hasNext()) {
            Hit strike;
            c284: strike = it.next();
            int x = strike.getX();
            ...
            if (pin != null) {
                ...
                c291: squaresStrike.put(new Square(x, y), pin);
            }
        }
