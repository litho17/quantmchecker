...
import powerbroker_1.edu.networkcusp.buyOp.bad.AuctionRaiser;
import powerbroker_1.edu.networkcusp.buyOp.bad.BadClaimRaiser;
import powerbroker_1.edu.networkcusp.buyOp.bad.IllegalOperationRaiser;
import powerbroker_1.edu.networkcusp.buyOp.bad.UnexpectedWinningOfferRaiser;
import powerbroker_1.edu.networkcusp.buyOp.bad.UnknownAuctionRaiser;
import powerbroker_1.edu.networkcusp.buyOp.bad.UnvalidatedWinnerRaiser;
...
public class AuctionProcessor {
    ...
    public synchronized void startAuction(String id, String description) throws ProtocolsRaiser {
        ...
        //save details
        @Inv("and (= auction.biddersStatus.biddersNotReported 0) (= auction.biddersStatus.claimedWinners 0) (= auction.biddersStatus.conceders 0) (= auction.proposals 0)") Auction auction = new Auction(id, myIdentity.fetchPublicIdentity(), description, true);
    ...
    public synchronized void closeAuction(String auctionId) throws ProtocolsRaiser, AuctionRaiser {
        ...
        if (!auctions.get(auctionId).amISeller()){
        ...
        auctions.get(auctionId).defineOver();
    ...
    public synchronized void announceWinner(String auctionId, String winner, int winningOffer) throws ProtocolsRaiser, AuctionRaiser {
        ...
        Integer expOffer = auctions.get(auctionId).grabExpectedWinningOffer(winner);
        ...
        if (!auctions.get(auctionId).verifyHighest(winningOffer)){
    ...
    public synchronized void processNewAuction(ProtocolsPublicIdentity customer, String auctionId, String description) throws IllegalOperationRaiser {
        if (!auctions.containsKey(auctionId)){
            @Inv("and (= auction.biddersStatus.biddersNotReported 0) (= auction.biddersStatus.claimedWinners 0) (= auction.biddersStatus.conceders 0) (= auction.proposals 0)") Auction auction = new Auction(auctionId, customer, description, false);
    ...
    public synchronized void makeOffer(String auctionId, int offer) throws Exception{
        ...
        if (auctions.containsKey(auctionId)){
            ...
            auctions.get(auctionId).recordMyCommit(myOfferData, myIdentity.fetchPublicIdentity());
            // send commitment out
    ...
    /**
     * STAC_ALT_NAMES: processCommit, processSubmission, processOffer, processPledge
     */
    public synchronized void processCommit(ProtocolsPublicIdentity customer, PromiseData commitData) throws AuctionRaiser, ProtocolsRaiser {
        ...
        if (auctions.containsKey(id)){
            @InvUnk("Nested lists") Auction auction = auctions.get(id);
    ...
    public synchronized void processTesting(ProtocolsPublicIdentity customer, ShareData compareData) throws ProtocolsRaiser, AuctionRaiser {
        ...
        boolean mineBig = compareData.isMineAsBig(auctions.get(auctionId).grabMyCommit());
        logger.info("Got comparison from " + customer.obtainTruncatedId() + " mineAsBig? " + mineBig);
        auctions.get(auctionId).addTesting(customer, mineBig);
        if (compareData.grabNeedReturn()){
            logger.info("BidComparison from " + customer.obtainTruncatedId() + " requested a response");
            PromiseData theirCommit = auctions.get(auctionId).grabOfferContract(customer);
            int myOffer = auctions.get(auctionId).grabMyCommit().pullOffer();
    ...
    public synchronized void biddingOver(ProtocolsPublicIdentity customer, String auctionId) throws ProtocolsRaiser, AuctionRaiser {
        auctions.get(auctionId).defineOver();
        if (auctions.get(auctionId).amIWinning()){
            logger.info("Sending claim");
            sendClaim(auctionId);
        }
        else if (auctions.get(auctionId).didIOffer() && !auctions.get(auctionId).amISeller()) {
    ...
    public synchronized void processWinClaim(ProtocolsPublicIdentity customer, String auctionId, OfferConveyData conveyData) throws ProtocolsRaiser, AuctionRaiser {
        ...
        PromiseData commit = (auctions.get(auctionId)).grabOfferContract(customer);
        int revealedOffer = conveyData.pullOffer();
        if (!(auctions.get(auctionId)).isOver()){
        ...
        if (!(auctions.get(auctionId)).isConsistentWithTesting(customer, revealedOffer)) {
            // check if we would have won if he hadn't lied in his comparison
            processWinClaimGuide(customer, auctionId, auctions.get(auctionId), revealedOffer);
            ...
            auctions.get(auctionId).removeOffer(customer); // disqualifyBid
            ...
            auctions.get(auctionId).addWinClaim(customer, revealedOffer);
        }
    }
    ...
    public synchronized void processConcession(ProtocolsPublicIdentity customer, String auctionId) throws UnknownAuctionRaiser {
        @InvUnk("Nested lists") Auction auction = auctions.get(auctionId);
        if (auction==null){
    ...
    public synchronized Map<String, String> obtainAuctionsStatusStrings(){
        @Bound("auctions") int i;
        @Inv("= (- auctionsStatus it) (- c323 c322)") Map<String, String> auctionsStatus = new TreeMap<String, String>();
        @Iter("<= it auctions") Iterator<String> it = auctions.keySet().iterator();
        while (it.hasNext()){
            String id;
            c322: id = it.next();
            c323: auctionsStatus.put(id, auctions.get(id).grabStatusString());
        }
    ...
    public synchronized BiddersStatus pullBiddersStatus(String auctionId)throws UnknownAuctionRaiser {
        ...
        return auctions.get(auctionId).pullBiddersStatus();
    }
    ...
    public synchronized void processAuctionEnd(ProtocolsPublicIdentity customer, String auctionId, String winner, int winningOffer) throws AuctionRaiser {
        ...
        if (!auctions.get(auctionId).verifySeller(customer)){ // make sure person announcing winner is the seller
        ...
        if (!winner.equals(myIdentity.fetchPublicIdentity().fetchId())){
            ...
            processAuctionEndHelper(customer, winner, winningOffer, auctions.get(auctionId));
        }
        // record winner
        auctions.get(auctionId).setWinner(winner, winningOffer);
    }
    ...
    private synchronized void sendClaim(String auctionId) throws ProtocolsRaiser, AuctionRaiser {
        OfferConveyData myConvey = auctions.get(auctionId).grabMyCommit();
    ...
    /**
     * STAC_ALT_NAMES: sendExchangeData, sendAnalysisData, sendShareData
     */
    ...
    /**
     * STAC_ALT_NAMES: checkAndSendExchangeData, verifyAndSendAnalysisData, checkAndSendShareData, auditAndSendExchangeData
     */
    ...
    /**
     * STAC_ALT_NAMES: checkExchangeData, verifyAnalysisData, checkShareBytes, reviewAnalysisData, auditExchangeData
     */
    ...
    /**
     * STAC_ALT_NAMES: checkData, verifyData, reviewData, auditData
     */
