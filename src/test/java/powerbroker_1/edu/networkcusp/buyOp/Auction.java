...
import powerbroker_1.edu.networkcusp.buyOp.bad.RebidRaiser;
...
public class Auction {
    ...
    public BiddersStatus biddersStatus = new BiddersStatusBuilder().createBiddersStatus();// data useful for seller in choosing winner (and deciding whether he should wait to do so)
    ...
    @Summary({"this.biddersStatus.biddersNotReported", "1", "this.proposals", "1"})
    ...
    @Summary({"this.biddersStatus.biddersNotReported", "2", "this.proposals", "1"})
    ...
    public void addTesting(ProtocolsPublicIdentity customer, boolean mineAsBig) throws IllegalOperationRaiser {
        ...
        if (data == null) {
            @Bound("+ proposals proposals") int i;
            @Inv("= (- builder it it) (- (+ c113 c114) c112 c112)") StringBuilder builder = new StringBuilder();
            @Iter("<= it proposals") Iterator<ProtocolsPublicIdentity> it = proposals.keySet().iterator();
            while (it.hasNext()) {
                ProtocolsPublicIdentity bidder;
                c112: bidder = it.next();
                c113: builder.append("have bidder " + bidder.toString());
                c114: builder.append('\n');
            }
    ...
    @Summary({"this.biddersStatus.conceders", "1"})
    ...
    public boolean verifyHighest(int claimedWinningOffer) {
        return biddersStatus.verifyHighest(claimedWinningOffer);
    }
