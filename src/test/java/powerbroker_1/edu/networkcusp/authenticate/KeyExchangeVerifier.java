...
public class KeyExchangeVerifier {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            KeyExchangeVerifierEntity.invoke();
        }
        try {
            KeyExchangeServer keyExchangeServer = new KeyExchangeServer(args[0]);
            String serversPublicKeyString = obtainServerKey(args[1]);
    ...
    private static String obtainServerKey(String serverKeyWalk) throws IOException {
        File serverKeyFile = new File(serverKeyWalk);
    ...
    private static class KeyExchangeVerifierEntity {
