...
public class PrivateCommsUtil {
    ...
    public static BigInteger toBigInt(byte[] data, int bitSize) {
        ...
        if (data.length > bitSize + 1) {
            return new PrivateCommsUtilSupervisor(data, bitSize).invoke();
        }
    ...
    public static byte[] fromBigInt(BigInteger dataInt, int bitSize) {
        ...
        if (result[0] == 0 && result.length > bitSize) {
            ...
            return fromBigIntWorker(result);
        }
    ...
    private static byte[] fromBigIntWorker(byte[] result) {
     ...
     */
    public static boolean verifySig(byte[] data, byte[] sig, PrivateCommsPublicKey publicKey) {
    ...
    private static class PrivateCommsUtilSupervisor {
        private byte[] data;
        private int bitSize;

        public PrivateCommsUtilSupervisor(byte[] data, int bitSize) {
            this.data = data;
