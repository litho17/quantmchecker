...
import powerbroker_1.edu.networkcusp.senderReceivers.ProtocolsClient;
...
import powerbroker_1.edu.networkcusp.senderReceivers.ProtocolsHandler;
...
import powerbroker_1.edu.networkcusp.senderReceivers.ProtocolsServer;
import powerbroker_1.edu.networkcusp.broker.PurchasePlan;
import powerbroker_1.edu.networkcusp.broker.ProductIntermediary;
import powerbroker_1.edu.networkcusp.broker.ProductIntermediaryRaiser;
import powerbroker_1.edu.networkcusp.broker.ProductIntermediaryCustomer;
...
public class StageOverseer implements ProtocolsHandler {
    ...
    public final List<ProtocolsConnection> connections = new ArrayList<>();
    public final List<ProtocolsNetworkAddress> peers = new ArrayList<>();
    ...
    public void handle(ProtocolsConnection connection, byte[] data) throws ProtocolsRaiser {
        ...
        } catch (@InvUnk("Extend library class") ProductIntermediaryRaiser e) {
            throw new ProtocolsRaiser(e);
        }
    }
    ...
    public void disconnectFromCustomers(List<ProtocolsNetworkAddress> addresses) {
        ...
        @Bound("connections") int i;
        @Inv("= (- addressToConnection c) (- c161 c163)") Map<ProtocolsNetworkAddress, ProtocolsConnection> addressToConnection = new HashMap<>();
        for (@Iter("<= c connections") int c = 0; c < connections.size(); ) {
            for (; (c < connections.size()) && (Math.random() < 0.5); ) {
                for (; (c < connections.size()) && (Math.random() < 0.5); ) {
                    for (; (c < connections.size()) && (Math.random() < 0.6); ) {
                        ProtocolsConnection connection = connections.get(c);
                        ProtocolsPublicIdentity identity = connection.takeTheirIdentity();
                        if (identity.hasCallbackAddress()) {
                            c161: addressToConnection.put(identity.takeCallbackAddress(), connection);
                        }
                        c163: c = c + 1;
                    }
                }
        ...
        for (int k = 0; k < addresses.size(); k++) {
            ProtocolsNetworkAddress address = addresses.get(k);
