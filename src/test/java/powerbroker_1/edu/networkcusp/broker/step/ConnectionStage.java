...
public class ConnectionStage extends Stage {
        ...
        try {
            ...
            try {
                if (numAttempts > 0) {
                    ...
                    if (connections.keySet().contains(other)) {
    ...
    protected Stage shouldTransitionToNextStage() throws ProductIntermediaryRaiser {
        ...
        if (pullNumReceivedFinalMessages() == peers.size() - 1) {
            ...
            @Bound("peers") int i;
            @Inv("= (- onlyPeers it) (- c143 c142 c145)") List<ProtocolsNetworkAddress> onlyPeers = new ArrayList<>();
            @Iter("<= it peers") Iterator<ProtocolsNetworkAddress> it = peers.iterator();
            while (it.hasNext()) {
                ProtocolsNetworkAddress p;
                c142: p = it.next();
                c143: onlyPeers.add(p);
            }
            c145: onlyPeers.remove(us);

            for (ProtocolsNetworkAddress address : connections.keySet()) {
            ...
            if (connections.keySet().containsAll(onlyPeers)) {
                return nextStage();
            }
