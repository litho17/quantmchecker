    ...
    public static void writeEmail(ReaderWriter writer, List<EmailEvent> eList, String outputuri) throws IOException {
        ...
        try {
            for (int i = 0; i < eList.size(); i++) {
                ...
                @InvUnk("Nested lists") List<EmailAddress> destinations = eList.get(i).getOtherdestination();
                ...
                @Inv("= (- wordList j) (- c76 c77)") List<String> wordList = new ArrayList<>();
                for (@Iter("<= j split") int j = 0; j < split.length;) {
                    c76: wordList.add(split[j]);
                    c77: j = j + 1;
                }
