...
public class AddressBook {
    ...
    public Set<EmailAddress> getAddresses(String pword) {
        ...
        Iterator<String> it = abook.keySet().iterator();
    ...
    public static String[] loadList(String fname) throws IOException {
        @Bound("+ 1 is") int i;
        @Inv("= (- list br) (- c174 c176 c173)") List<String> list = new ArrayList();
        ClassLoader classloader = Thread.currentThread().getContextClassLoader();
        InputStream is = classloader.getResourceAsStream("data/" + fname);

        try (@Iter("<= br is") BufferedReader br = new BufferedReader(new InputStreamReader(is, "UTF-8"))) {
            String line;
            c173: line = br.readLine();
            while (line != null) {
                c174: list.add(line);
                c176: line = br.readLine();
            }
        }
