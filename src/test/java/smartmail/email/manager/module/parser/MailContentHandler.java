...
public class MailContentHandler implements ContentHandler {
    ...
    public void body(BodyDescriptor body, InputStream is) throws MimeException,
        ...
        @Bound("is") int i;
        @Inv("= (- out in) (- c74 c70)") final StringBuilder out = new StringBuilder();
        @Iter("<= in is") Reader in = new InputStreamReader(is, "UTF-8");
        for (;;) {
            int rsz;
            c70: rsz = in.read(buffer, 0, buffer.length);
            if (rsz < 0) {
                break;
            }
            c74: out.append(buffer, 0, rsz);
        }
