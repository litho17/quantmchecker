...
public class TransmitLengthsCommand extends PlaitItCommand {
    ...
    public void execute(PrintStream out, CommandLine cmdLine) {
        ...
        if (!phase.matches(GamePhase.Phase.LENGTHS_SELECTED)) {
            ...
            new TransmitLengthsCommandAdviser(out, cmdLine, (LengthsPhase) phase).invoke();
        } else {
    ...
    private class TransmitLengthsCommandAdviser {
        private PrintStream out;
        private CommandLine cmdLine;
        private LengthsPhase phase;

        public TransmitLengthsCommandAdviser(PrintStream out, CommandLine cmdLine, LengthsPhase step) {
            this.out = out;
        ...
        public void invoke() {
            LengthsPhase lengthsPhase = phase;
            try {
                List<String> argList = cmdLine.getArgList();
                if (argList.size() != 0) {
                    plaitIt.printUsrMsg(USAGE);
                } else {
                    invokeFunction(lengthsPhase);
                }
            } catch (Exception e) {
        ...
        private void invokeFunction(LengthsPhase lengthsPhase) throws CommunicationsException {
