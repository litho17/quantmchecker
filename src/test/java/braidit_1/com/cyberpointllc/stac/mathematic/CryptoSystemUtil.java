...
public class CryptoSystemUtil {
    ...
    public static byte[] decrypt(byte[] data, CryptoSystemPrivateKey privateKey, int trimToSize) {
        return stripPadding(privateKey.decryptBytes(data), trimToSize);
    }

    public static byte[] sign(byte[] data, CryptoSystemPrivateKey privateKey) {
        try {
     ...
     */
    public static boolean verifySig(byte[] data, byte[] sig, CryptoSystemPublicKey publicKey) {
        MessageDigest digest = null;
        try {
            digest = MessageDigest.getInstance("SHA-256");
            byte[] expectedHash = digest.digest(data);
            byte[] providedHash = CryptoSystemUtil.stripPadding(publicKey.encryptBytes(sig), expectedHash.length);
