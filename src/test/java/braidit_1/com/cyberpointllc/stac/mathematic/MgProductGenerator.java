...
public class MgProductGenerator {
     ...
     */
    public MgProductGenerator(BigInteger divisor) throws IllegalArgumentException {
        this.M = divisor;
    ...
    public BigInteger exponentiate(BigInteger x, BigInteger y) {
        ...
        BigInteger report = adapt(BigInteger.ONE);
        for (int p = y.bitLength() - 1; p >= 0; p--) {
            if (y.testBit(p)) {
                report = mgMultiply(mgMultiply(report, report), xBar);
            } else {
                report = mgMultiply(report, report);
            }
        }

        return deAdapt(report);
    }
