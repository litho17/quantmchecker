...
class CFRelation(tree: StatementTree, solver: Z3Solver) {
  ...
  private def genEqCons(stmts: Set[Tree]): List[AST] = {
    if (stmts.nonEmpty) {
      ...
      val head = solver.getVar(headHashcode)
      stmts.tail.foldLeft(List[AST]()) {
        (acc, stmt) => // head = s1, head = s2, ...
          val eq = solver.mkEq(head, solver.getVar(Utils.hashCode(stmt)))
          eq :: acc
      }
    } else List.empty
  }
  ...
  def treeToCons(tree: StatementTree): List[AST] = {
    ...
    def genLoopCons(loop: StatementTree, body: StatementTree): List[AST] = {
      if (loop.isInstanceOf[DoWhileLoopTree] || loop.isInstanceOf[EnhancedForLoopTree] || loop.isInstanceOf[WhileLoopTree]) { // block >= this
        val bodyVar = solver.getVar(Utils.hashCode(body))
        val loopVar = solver.getVar(Utils.hashCode(loop))
    ...
    val me = solver.getVar(thisHashcode)
    tree match {
      case stmt: BlockTree =>
      ...
      case stmt: IfTree => // this = then + else
        val thenVar = solver.getVar(Utils.hashCode(stmt.getThenStatement))
        val eq =
          if (stmt.getElseStatement != null) {
            val elseVar = solver.getVar(Utils.hashCode(stmt.getElseStatement))
      ...
      case stmt: SwitchTree => // this = case1 + case2 + ...
        val cases = {
          ...
          if (cases.size == 1) solver.getVar(Utils.hashCode(cases.head))
          else solver.mkAdd(cases.map(casetree => solver.getVar(Utils.hashCode(casetree))).toArray: _*)
        }
      ...
      case stmt: TryTree => // this = try; this = finally; this = catch1 + catch2 + ...
        val tryVar = solver.getVar(Utils.hashCode(stmt.getBlock))
        val eq1 = solver.mkEq(me, tryVar)
        val eq2 =
          if (stmt.getFinallyBlock != null) {
            val finallyVar = solver.getVar(Utils.hashCode(stmt.getFinallyBlock))
            solver.mkEq(me, finallyVar)
          } else solver.ctx.mkTrue()
        val eq3 = {
          val catches = stmt.getCatches.asScala
          if (catches.isEmpty) solver.ctx.mkTrue()
          else if (catches.size == 1) solver.mkEq(me, solver.getVar(Utils.hashCode(catches.head)))
          else solver.mkEq(me, solver.mkAdd(catches.map(catchtree => solver.getVar(Utils.hashCode(catchtree))).toArray: _*))
        }
